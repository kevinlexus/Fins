...
uses
  ...
  StdCtrls, Mask, wwdbedit, Wwdotdot, Wwdbcomb, wwdblook, DBCtrls, GridsEh,
  DBGridEh, frxClass, frxDBSet, Menus, Unit_ecr, Math;
...
type
  TForm_get_pay_nal = class(TForm)
    ...
    OD_c_kwtpDEP: TFloatField;
    OD_c_kwtpFIO_OWNER: TStringField;
    ...
    procedure DBGridEh1Columns2EditButtonDown(Sender: TObject; TopButton: Boolean; var AutoRepeat, Handled: Boolean);
...
uses
  ...
  Unit_get_cnt_sch, DM_module1, Unit_list_kart;
...
begin
  ...
  begin
    l_nkom := DataModule1.OraclePackage1.CallStringFunction('scott.UTILS.get_nkom_pay_lsk', [wwDBEdit3.Text]);
    ;
...
begin
  ...
  begin
    DBGridEh1.Visible := true;
    DBGridEh1.SetFocus;
    Edit1.Text := DataModule1.OraclePackage1.CallStringFunction('scott.UTILS.GET_ADR_BY_LSK', [wwDBEdit3.Text]);
    Form_main.c_lsk_id_ := DataModule1.OraclePackage1.CallStringFunction('scott.UTILS.GET_C_LSK_ID_BY_LSK', [wwDBEdit3.Text]);
...
begin
  ...
  begin
    ...
    begin
      ...
      Form_get_cnt_sch.wwDBEdit4.Text := FloatToStr(DataModule1.OraclePackage1.CallFloatFunction('scott.c_charges.gen_charges_sch', [wwDBEdit3.Text, OD_usl_chk.FieldByName('fk_usl_chk').AsString, 0, null]));
      Form_get_cnt_sch.cnt_sch0_ := DataModule1.OraclePackage1.CallFloatFunction('scott.c_charges.gen_charges_sch', [wwDBEdit3.Text, OD_usl_chk.FieldByName('fk_usl_chk').AsString, 0, null]);
    end
    else
    begin
...
begin
  ...
  begin
    ...
    Button1.SetFocus;
    summa_ := RoundTo(summGet - summItg, -2);
...
//       DataModule1.OracleSession1.ApplyUpdates([OD_chargepay], false);
        ...
        err_ := DataModule1.OraclePackage1.CallIntegerFunction('scott.C_GET_PAY.dst_money_cur_month', [summa_]);
        if err_ = 0 then
        begin
          summa_ := 0;
          OD_chargepay.Active := False;
...
var
  i, l_par, c_kwtp_id_, l_flag, mode: Integer;
...
begin
  ...
  c_kwtp_id_ := DataModule1.OraclePackage1.CallIntegerFunction('scott.C_GET_PAY.get_money_nal', [wwDBEdit3.Text]);
  ...
  begin
    try
      ...
      begin
        ...
        // проверить режим
        mode:=check_mode();
        if (Form_main.have_cash <> 2) or ((Form_main.have_cash=2) and (mode = 2) or (mode = 3))  then // если ККМ=2 то проверить режимы
        begin
          // открыть чек
          if open_reg <> 0 then
          begin
          ...
          begin
              print_string_ecr('Вас обслуживает организация:' + OD_c_kwtp.FieldByName('name_org2').AsString + ' ИНН ' + OD_c_kwtp.FieldByName('inn2').AsString + ', тел.' + OD_c_kwtp.FieldByName('phone2').AsString, 1, 0);
              ...
              print_string_ecr('Отдел №' + OD_c_kwtp.FieldByName('dep').AsString, 1, 0);
              print_string_ecr('---------------------------------------', 1, 0);
              print_string_ecr('Лицевой счет №' + OD_c_kwtp.FieldByName('lsk').AsString, 1, 0);
              print_string_ecr('Ф.И.О.:' + OD_c_kwtp.FieldByName('fio_owner').AsString, 1, 0);
              ...
              begin
                reg_ecr(OD_c_kwtp_mg.FieldByName('naim').AsString + calc_pads(OD_c_kwtp_mg.FieldByName('naim').AsString) + OD_c_kwtp_mg.FieldByName('dopl').AsString, OD_c_kwtp_mg.FieldByName('summ_itg').AsFloat, 1, OD_c_kwtp.FieldByName('dep').AsInteger);
                OD_c_kwtp_mg.Next;
              end;
              //Закрытие чека
        ...
        begin
            ...
            l_flag:=1;
            msg2('Ошибка, ККМ находится в некорректном режиме:'+check_mode2(),
...
procedure TForm_get_pay_nal.DBGridEh1Columns2EditButtonClick(Sender: TObject; var Handled: Boolean);
begin
  action;
end;

procedure TForm_get_pay_nal.DBGridEh1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TForm_get_pay_nal.DBGridEh1Columns2EditButtonDown(Sender: TObject; TopButton: Boolean; var AutoRepeat, Handled: Boolean);
begin
  action;
end;
...
begin
  if ((DBGridEh1.SelectedField.FieldName = 'SUMMA') or (DBGridEh1.SelectedField.FieldName = 'PENYA')) then
  begin
