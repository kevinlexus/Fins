       ...
       begin
        try
          ...
          begin
            ...
            while not Eof do
            begin
                unreg_ecr(FieldByName('oper_name').AsString+calc_pads(FieldByName('oper_name').AsString)+
...
if msg3('Выполнить обратный платёж оплаты с суммой '+OD_c_kwtp.FieldByName('summ_itg').AsString+
   ' руб., Л/С '+OD_c_kwtp.FieldByName('lsk').AsString+' (действие будет сохранено в журнале событий)',
   'Внимание!',
   MB_YESNO+MB_ICONQUESTION) = IDYES then
   begin
   ...
   begin
     ...
     l_var:=DataModule1.OraclePackage1.CallIntegerFunction
      ...
      OD_paycheck.Active:=false;
      DataModule1.OraclePackage1.Session.Commit;
      msg2('Обратный платеж выполнен в текущем дне и с текущим № компьютера!', 'Внимание!',
        MB_OK+MB_ICONINFORMATION)
   end;
   end;
