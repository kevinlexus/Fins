object Form_det_chrg: TForm_det_chrg
  Left = 237
  Top = 403
  Width = 779
  Height = 534
  Caption = #1044#1077#1090#1072#1083#1080#1079#1072#1094#1080#1103' '#1088#1072#1089#1095#1105#1090#1072
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object wwDBGrid1: TwwDBGrid
    Left = 0
    Top = 0
    Width = 771
    Height = 458
    Selected.Strings = (
      'RN'#9'5'#9#8470' ~'#1087#1086#1088#1103#1076#1082'.'#9'F'
      'NM'#9'15'#9#1059#1089#1083#1091#1075#1072#9'F'
      'TEST_OPL'#9'6'#9#1054#1073#1098#1105#1084#9'F'
      'TEST_CENA'#9'9'#9#1062#1077#1085#1072#9'F'
      'ED_IZM'#9'5'#9#1045#1076'.'#1080#1079#1084'.'#9'F'
      'SUMMA_TR'#9'9'#9#1057#1091#1084#1084#1072' ~'#1090#1072#1088#9'F'
      'SUMMA_LG'#9'9'#9#1057#1091#1084#1084#1072' ~'#1083#1100#1075#9'F'
      'SUMMA_SUBS'#9'7'#9#1057#1091#1084#1084#1072' ~'#1089#1091#1073#1089#9'F'
      'SUMMA_ITG'#9'7'#9#1048#1090#1086#1075#1086#9'F'
      'FIO'#9'24'#9#1060'.'#1048'.'#1054'.'#9'F'
      'LG_NAME'#9'18'#9#1051#1100#1075#1086#1090#1072#9'F')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    Align = alClient
    DataSource = DS_c_charge
    KeyOptions = [dgEnterToTab]
    Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgPerfectRowFit, dgShowFooter, dgShowCellHint, dgTabExitsOnLastCol, dgProportionalColResize, dgDblClickColSizing]
    ReadOnly = True
    TabOrder = 0
    TitleAlignment = taLeftJustify
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    TitleLines = 2
    TitleButtons = False
    UseTFields = False
    FooterHeight = 20
    PaintOptions.ActiveRecordColor = clBackground
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 458
    Width = 771
    Height = 49
    Align = alBottom
    TabOrder = 1
    object Button4: TButton
      Left = 686
      Top = 16
      Width = 75
      Height = 25
      Cancel = True
      Caption = #1047#1072#1082#1088#1099#1090#1100
      TabOrder = 1
      OnClick = Button4Click
    end
    object Button1: TButton
      Left = 598
      Top = 16
      Width = 75
      Height = 25
      Cancel = True
      Caption = #1055#1077#1095#1072#1090#1100
      TabOrder = 0
      OnClick = Button1Click
    end
  end
  object OD_c_charge: TOracleDataSet
    SQL.Strings = (
      'select rownum as rn, a.* from ('
      
        'select t.npp, t.test_opl, t.test_cena, u.ed_izm, t.summa as summ' +
        'a_tr, t2.summa as summa_lg, t3.summa as summa_subs,'
      ' nvl(t.summa,0)-nvl(t2.summa,0)-nvl(t3.summa,0) as summa_itg, '
      ' u.nm, '
      ' c.fio, decode(t2.spk_id, 1, '#39#39', t2.name) as lg_name'
      '   from scott.c_charge t, '
      '    (select t.*, s.name from scott.c_charge t, scott.spk s '
      '     where t.lsk=:lsk and t.spk_id=s.id and t.type=4) t2, '
      '    (select t.*, s.name from scott.c_charge t, scott.spk s '
      '     where t.lsk=:lsk and t.spk_id=s.id and t.type=2) t3, '
      '    scott.usl u, scott.c_kart_pr c'
      '    where t.lsk=:lsk and t.usl=u.usl and t.type=1 and '
      '    t.lsk=t2.lsk(+) and t.usl=t2.usl(+) and t.npp=t2.npp(+) and'
      '    t.lsk=t3.lsk(+) and t.usl=t3.usl(+) and t.npp=t3.npp(+) '
      '    and t.kart_pr_id=c.id(+) and t.summa <> 0 '
      '    order by t.usl, t.npp, t.type'
      ') a')
    Optimize = False
    Variables.Data = {
      0300000001000000040000003A4C534B05000000090000002020202020202020
      0000000000}
    QBEDefinition.QBEFieldDefs = {
      
      5F43454E41010000000000}
    Master = Form_list_kart.OD_list_kart
    MasterFields = 'lsk'
    DetailFields = 'lsk'
    Session = DataModule1.OracleSession1
    Active = True
    Left = 16
    Top = 64
  end
  object DS_c_charge: TDataSource
    DataSet = OD_c_charge
    Left = 48
    Top = 64
  end
  object frxDBDataset1: TfrxDBDataset
    UserName = 'frxDBData_charge'
    CloseDataSource = False
    FieldAliases.Strings = (
      'RN=RN'
      'NPP=NPP'
      'TEST_OPL=TEST_OPL'
      'ED_IZM=ED_IZM'
      'SUMMA_TR=SUMMA_TR'
      'SUMMA_LG=SUMMA_LG'
      'SUMMA_SUBS=SUMMA_SUBS'
      'SUMMA_ITG=SUMMA_ITG'
      'NM=NM'
      'FIO=FIO'
      'LG_NAME=LG_NAME')
    DataSet = OD_c_charge
    BCDToCurrency = False
    Left = 16
    Top = 96
  end
  object frxReport1: TfrxReport
    Version = '0.0'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = #1055#1086' '#1091#1084#1086#1083#1095#1072#1085#1080#1102
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40087.404347916700000000
    ReportOptions.LastChange = 40087.414824259300000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      'begin'
      ''
      'end.')
    Left = 48
    Top = 96
    Datasets = <
      item
        DataSet = frxDBDataset1
        DataSetName = 'frxDBData_charge'
      end>
    Variables = <>
    Style = <
      item
        Name = 'Title'
        Color = 14211288
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Arial'
        Font.Style = [fsBold]
      end
      item
        Name = 'Header'
        Color = 15790320
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
      end
      item
        Name = 'Group header'
        Color = 15790320
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
      end
      item
        Name = 'Data'
        Color = clNone
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
      end
      item
        Name = 'Group footer'
        Color = clNone
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        Frame.Typ = [ftTop]
      end
      item
        Name = 'Header line'
        Color = clNone
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        Frame.Width = 2.000000000000000000
      end>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        Height = 26.456710000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          Align = baWidth
          Width = 718.110700000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 14211288
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#8221#1056#181#1057#8218#1056#176#1056#187#1056#1105#1056#183#1056#176#1057#8224#1056#1105#1057#1039' '#1057#1026#1056#176#1057#1027#1057#8225#1056#181#1057#8218#1056#176)
          ParentFont = False
          Style = 'Title'
          VAlign = vaCenter
        end
      end
      object PageHeader1: TfrxPageHeader
        Height = 34.015770000000000000
        Top = 68.031540000000010000
        Width = 718.110700000000000000
        object Memo3: TfrxMemoView
          Width = 26.951523800000000000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1074#8222#8211' '#1056#1111'/'#1056#1111)
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Left = 147.896483800000000000
          Width = 43.466583070000000000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#1115#1056#177#1057#1033#1057#8216#1056#1112)
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Left = 187.583536870000000000
          Width = 26.543290000000000000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#8226#1056#1169'.'#1056#1105#1056#183#1056#1112'.')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 214.126826870000000000
          Width = 52.434467457575590000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#1038#1057#1107#1056#1112#1056#1112#1056#176' '#1057#8218#1056#176#1057#1026'.')
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          Left = 266.561294327575600000
          Width = 52.434467457575590000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#1038#1057#1107#1056#1112#1056#1112#1056#176' '#1056#187#1057#1034#1056#1110#1056#1109#1057#8218)
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 318.995761775781200000
          Width = 64.232222631782120000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#1038#1057#1107#1056#1112#1056#1112#1056#176' '#1057#1027#1057#1107#1056#177#1057#1027'.')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 383.227984416933300000
          Width = 55.733412453066620000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#152#1057#8218#1056#1109#1056#1110#1056#1109)
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 26.992626870000000000
          Width = 118.786293630000000000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#1032#1057#1027#1056#187#1057#1107#1056#1110#1056#176)
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 440.582260500000000000
          Width = 135.107836600000000000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#164'.'#1056#152'.'#1056#1115'.')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 575.690097100000000000
          Width = 138.640609120000000000
          Height = 30.236240000000000000
          ShowHint = False
          Color = 15790320
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            #1056#1113#1056#176#1057#8218#1056#181#1056#1110#1056#1109#1057#1026#1056#1105#1057#1039' '#1056#187#1057#1034#1056#1110#1056#1109#1057#8218)
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        Height = 18.897650000000000000
        Top = 162.519790000000000000
        Width = 718.110700000000000000
        DataSet = frxDBDataset1
        DataSetName = 'frxDBData_charge'
        RowCount = 0
        Stretched = True
        object Memo13: TfrxMemoView
          Width = 26.951523800000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'RN'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBData_charge."RN"]')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Left = 144.116953800000000000
          Width = 43.466583070000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'TEST_OPL'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[frxDBData_charge."TEST_OPL"]')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 187.583536870000000000
          Width = 30.322820000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'ED_IZM'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[frxDBData_charge."ED_IZM"]')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Left = 217.906356870000000000
          Width = 52.180687840000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'SUMMA_TR'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[frxDBData_charge."SUMMA_TR"]')
          ParentFont = False
        end
        object Memo17: TfrxMemoView
          Left = 270.087044710000000000
          Width = 55.960217840000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'SUMMA_LG'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[frxDBData_charge."SUMMA_LG"]')
          ParentFont = False
        end
        object Memo18: TfrxMemoView
          Left = 326.047262540000000000
          Width = 55.960217840000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'SUMMA_SUBS'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[frxDBData_charge."SUMMA_SUBS"]')
          ParentFont = False
        end
        object Memo19: TfrxMemoView
          Left = 383.259939400000000000
          Width = 55.960217840000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'SUMMA_ITG'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBData_charge."SUMMA_ITG"]')
          ParentFont = False
        end
        object Memo20: TfrxMemoView
          Left = 26.992626870000000000
          Width = 115.006763630000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'NM'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBData_charge."NM"]')
          ParentFont = False
        end
        object Memo21: TfrxMemoView
          Left = 440.582260500000000000
          Width = 135.107836600000000000
          Height = 18.897650000000000000
          ShowHint = False
          StretchMode = smActualHeight
          DataField = 'FIO'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBData_charge."FIO"]')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          Left = 575.690097100000000000
          Width = 138.640609120000000000
          Height = 18.897650000000000000
          ShowHint = False
          StretchMode = smActualHeight
          DataField = 'LG_NAME'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBData_charge."LG_NAME"]')
          ParentFont = False
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 26.456710000000000000
        Top = 287.244280000000000000
        Width = 718.110700000000000000
        object Memo23: TfrxMemoView
          Align = baWidth
          Width = 718.110700000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
          Frame.Width = 2.000000000000000000
        end
        object Memo24: TfrxMemoView
          Top = 1.000000000000000000
          Height = 22.677180000000000000
          ShowHint = False
          AutoWidth = True
          Memo.UTF8 = (
            '[Date] [Time]')
        end
        object Memo25: TfrxMemoView
          Align = baRight
          Left = 642.520100000000000000
          Top = 1.000000000000000000
          Width = 75.590600000000000000
          Height = 22.677180000000000000
          ShowHint = False
          HAlign = haRight
          Memo.UTF8 = (
            #1056#1038#1057#8218#1057#1026'. [Page#]')
        end
      end
      object Footer1: TfrxFooter
        Height = 22.677180000000000000
        Top = 204.094620000000000000
        Width = 718.110700000000000000
        object Memo26: TfrxMemoView
          Left = 211.653680000000000000
          Top = 3.779529999999994000
          Width = 60.417881230000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[SUM(<frxDBData_charge."SUMMA_TR">,MasterData1)]')
          ParentFont = False
        end
        object Memo27: TfrxMemoView
          Left = 272.274865430000000000
          Top = 3.779529999999994000
          Width = 52.858821229999990000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[SUM(<frxDBData_charge."SUMMA_LG">,MasterData1)]')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          Left = 325.336990860000000000
          Top = 3.779529999999994000
          Width = 56.638351230000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8 = (
            '[SUM(<frxDBData_charge."SUMMA_SUBS">,MasterData1)]')
          ParentFont = False
        end
        object Memo29: TfrxMemoView
          Left = 383.629428770000000000
          Top = 3.779529999999994000
          Width = 56.638351230000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[SUM(<frxDBData_charge."SUMMA_ITG">,MasterData1)]')
          ParentFont = False
        end
        object Memo30: TfrxMemoView
          Left = 3.779530000000000000
          Top = 3.779529999999994000
          Width = 197.311923410000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBData_charge'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            #1056#152#1057#8218#1056#1109#1056#1110#1056#1109':')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Width = 714.331170000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
      end
    end
  end
end
