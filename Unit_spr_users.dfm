object Form_spr_users: TForm_spr_users
  Left = 317
  Top = 411
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1080' - '#1088#1072#1079#1088#1077#1096#1077#1085#1080#1103
  ClientHeight = 660
  ClientWidth = 879
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter1: TSplitter
    Left = 281
    Top = 49
    Width = 8
    Height = 611
    ExplicitLeft = 217
    ExplicitHeight = 362
  end
  object GroupBox2: TGroupBox
    Left = 0
    Top = 49
    Width = 281
    Height = 611
    Align = alLeft
    Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1080
    TabOrder = 1
    ExplicitHeight = 684
    object cxGrid1: TcxGrid
      Left = 2
      Top = 15
      Width = 277
      Height = 594
      Align = alClient
      TabOrder = 0
      ExplicitHeight = 667
      object cxGrid1DBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Visible = True
        DataController.DataSource = DS_c_users2
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsBehavior.FocusCellOnTab = True
        OptionsBehavior.GoToNextCellOnEnter = True
        OptionsData.CancelOnExit = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.GroupByBox = False
        OptionsView.Indicator = True
        Styles.Inactive = cxStyle2
        object cxGrid1DBTableView1ID: TcxGridDBColumn
          DataBinding.FieldName = 'ID'
          Width = 20
        end
        object cxGrid1DBTableView1USERNAME: TcxGridDBColumn
          Caption = #1051#1086#1075#1080#1085
          DataBinding.FieldName = 'CD'
          Width = 53
        end
        object cxGrid1DBTableView1FIO: TcxGridDBColumn
          Caption = #1060'.'#1048'.'#1054'.'
          DataBinding.FieldName = 'NAME'
          Width = 73
        end
        object cxGrid1DBTableView1GUID: TcxGridDBColumn
          DataBinding.FieldName = 'GUID'
          Width = 129
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
  end
  object GroupBox3: TGroupBox
    Left = 289
    Top = 49
    Width = 221
    Height = 611
    Align = alClient
    Caption = #1044#1086#1089#1090#1091#1087#1085#1086' '#1087#1086' '#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103#1084
    TabOrder = 2
    ExplicitWidth = 210
    ExplicitHeight = 684
    object cxGrid2: TcxGrid
      Left = 2
      Top = 15
      Width = 217
      Height = 594
      Align = alClient
      PopupMenu = PopupMenu1
      TabOrder = 0
      ExplicitWidth = 206
      ExplicitHeight = 667
      object cxgridAvailable: TcxGridDBTableView
        OnDragDrop = cxgridAvailableDragDrop
        OnDragOver = cxgridAvailableDragOver
        Navigator.Buttons.CustomButtons = <>
        Navigator.Visible = True
        DataController.DataSource = DS_user_perm2
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.GroupByBox = False
        object cxgridAvailableNAME_REU: TcxGridDBColumn
          DataBinding.FieldName = 'NAME_REU'
          Width = 176
        end
        object cxgridAvailableNAME_ORG: TcxGridDBColumn
          DataBinding.FieldName = 'NAME_ORG'
          Width = 149
        end
        object cxgridAvailableFK_COMP: TcxGridDBColumn
          DataBinding.FieldName = 'FK_COMP'
          Width = 20
        end
      end
      object cxGridLevel1: TcxGridLevel
        GridView = cxgridAvailable
      end
    end
  end
  object GroupBox4: TGroupBox
    Left = 510
    Top = 49
    Width = 369
    Height = 611
    Align = alRight
    Caption = #1042#1089#1077' '#1086#1073#1098#1077#1082#1090#1099
    TabOrder = 3
    ExplicitLeft = 499
    ExplicitHeight = 684
    object cxGrid3: TcxGrid
      Left = 2
      Top = 15
      Width = 365
      Height = 594
      Align = alClient
      TabOrder = 0
      ExplicitHeight = 667
      object cxgridAllObjects: TcxGridDBTableView
        OnMouseDown = cxgridAllObjectsMouseDown
        Navigator.Buttons.CustomButtons = <>
        Navigator.Visible = True
        DataController.DataSource = DS_s_reu_trest2
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.GroupByBox = False
        object cxgridAllObjectsNAME_ORG: TcxGridDBColumn
          DataBinding.FieldName = 'NAME_ORG'
        end
        object cxgridAllObjectsNKOM: TcxGridDBColumn
          DataBinding.FieldName = 'NKOM'
          Visible = False
        end
      end
      object cxGridLevel2: TcxGridLevel
        GridView = cxgridAllObjects
      end
    end
  end
  object GroupBox5: TGroupBox
    Left = 0
    Top = 0
    Width = 879
    Height = 49
    Align = alTop
    Caption = #1042#1080#1076' '#1076#1086#1089#1090#1091#1087#1072
    TabOrder = 0
    ExplicitWidth = 868
    object cxLookupComboBox1: TcxLookupComboBox
      Left = 13
      Top = 17
      Properties.GridMode = True
      Properties.KeyFieldNames = 'ID'
      Properties.ListColumns = <
        item
          FieldName = 'NAME'
        end>
      Properties.ListOptions.ShowHeader = False
      Properties.ListSource = DS_permtp2
      Properties.OnCloseUp = cxLookupComboBox1PropertiesCloseUp
      TabOrder = 0
      Width = 436
    end
  end
  object OD_s_reu_trest: TOracleDataSet
    SQL.Strings = (
      
        'select null as id, null as nkom, s.reu, s.reu||'#39'-'#39'||s.name_reu a' +
        's name_org'
      '  from scott.s_reu_trest s'
      'order by s.reu'
      '  ')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0500000004000000060000005200450055000100000000000400000049004400
      010000000000100000004E0041004D0045005F004F0052004700010000000000
      080000004E004B004F004D00010000000000}
    ReadOnly = True
    RefreshOptions = [roAfterInsert]
    CommitOnPost = False
    Session = DataModule1.OracleSession1
    Left = 672
    Top = 160
    object OD_s_reu_trestNAME_ORG: TStringField
      DisplayLabel = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
      DisplayWidth = 35
      FieldName = 'NAME_ORG'
      Size = 35
    end
    object OD_s_reu_trestNKOM: TStringField
      DisplayWidth = 1
      FieldName = 'NKOM'
      Visible = False
      Size = 1
    end
    object OD_s_reu_trestID: TStringField
      DisplayWidth = 1
      FieldName = 'ID'
      Visible = False
      Size = 1
    end
    object OD_s_reu_trestREU: TStringField
      DisplayWidth = 3
      FieldName = 'REU'
      Required = True
      Visible = False
      Size = 3
    end
  end
  object OD_user_perm: TOracleDataSet
    SQL.Strings = (
      
        'select c.*, c.rowid, t.reu||'#39'-'#39'||t.name_reu as name_reu, o.name ' +
        'as name_org'
      'from scott.c_users_perm c, scott.s_reu_trest t, scott.t_org o'
      'where c.user_id = :user_id and c.fk_perm_tp=:permtp'
      'and c.fk_reu=t.reu(+) and c.fk_pasp_org=o.id(+)'
      'order by t.reu, o.name')
    Optimize = False
    Variables.Data = {
      0400000002000000100000003A0055005300450052005F004900440004000000
      080000000000000000006C40000000000E0000003A005000450052004D005400
      5000030000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      
      0400000049004400010000000000}
    ReadOnly = True
    RefreshOptions = [roAfterInsert]
    CommitOnPost = False
    Session = DataModule1.OracleSession1
    Active = True
    Left = 344
    Top = 128
    object OD_user_permNAME_REU: TStringField
      DisplayLabel = #1054#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103
      DisplayWidth = 31
      FieldName = 'NAME_REU'
      Size = 35
    end
    object OD_user_permNAME_ORG: TStringField
      DisplayLabel = #1054#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103
      DisplayWidth = 64
      FieldName = 'NAME_ORG'
      Size = 64
    end
    object OD_user_permFK_COMP: TStringField
      DisplayLabel = #1050#1086#1084#1087#1100#1102#1090#1077#1088
      DisplayWidth = 3
      FieldName = 'FK_COMP'
      Size = 3
    end
    object OD_user_permFK_REU: TStringField
      FieldName = 'FK_REU'
      Visible = False
      Size = 3
    end
    object OD_user_permFK_PASP_ORG: TFloatField
      FieldName = 'FK_PASP_ORG'
      Visible = False
    end
  end
  object OD_t_org: TOracleDataSet
    SQL.Strings = (
      
        'select t.id, null as nkom, null as reu, t.name as name_org from ' +
        'scott.t_org t, scott.t_org_tp p'
      'where t.fk_orgtp=p.id and p.cd='#39#1055#1072#1089#1087#1086#1088#1090#1085#1099#1081' '#1089#1090#1086#1083#39)
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0500000004000000060000005200450055000100000000000400000049004400
      010000000000100000004E0041004D0045005F004F0052004700010000000000
      080000004E004B004F004D00010000000000}
    ReadOnly = True
    RefreshOptions = [roAfterInsert]
    CommitOnPost = False
    Session = DataModule1.OracleSession1
    Active = True
    Left = 672
    Top = 208
    object OD_t_orgID: TFloatField
      FieldName = 'ID'
      Required = True
    end
    object OD_t_orgREU: TStringField
      FieldName = 'REU'
      Size = 1
    end
    object OD_t_orgNAME_ORG: TStringField
      FieldName = 'NAME_ORG'
      Required = True
      Size = 64
    end
    object OD_t_orgNKOM: TStringField
      FieldName = 'NKOM'
      Size = 1
    end
  end
  object OD_permtp: TOracleDataSet
    SQL.Strings = (
      
        'select t.id, t.cd, t.name as name from scott.u_list t, scott.u_l' +
        'isttp tp'
      'where t.fk_listtp=tp.id and tp.cd='#39#1058#1080#1087' '#1088#1072#1079#1088#1077#1096#1077#1085#1080#1103#39
      'order by t.name')
    Optimize = False
    SequenceField.ApplyMoment = amOnNewRecord
    QBEDefinition.QBEFieldDefs = {
      0500000003000000040000004900440001000000000004000000430044000100
      00000000080000004E0041004D004500010000000000}
    Session = DataModule1.OracleSession1
    Active = True
    Left = 344
    Top = 200
    object OD_permtpID: TFloatField
      FieldName = 'ID'
      Required = True
    end
    object OD_permtpNAME: TStringField
      FieldName = 'NAME'
      Required = True
      Size = 96
    end
    object OD_permtpCD: TStringField
      FieldName = 'CD'
      Required = True
      Size = 32
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 329
    Top = 393
    object N1: TMenuItem
      Caption = #1059#1076#1072#1083#1080#1090#1100
      OnClick = N1Click
    end
  end
  object OD_c_comps: TOracleDataSet
    SQL.Strings = (
      
        'select null as id, nkom, null as reu, t.nkom as name_org from sc' +
        'ott.c_comps t')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0500000004000000060000005200450055000100000000000400000049004400
      010000000000100000004E0041004D0045005F004F0052004700010000000000
      080000004E004B004F004D00010000000000}
    ReadOnly = True
    RefreshOptions = [roAfterInsert]
    CommitOnPost = False
    Session = DataModule1.OracleSession1
    Active = True
    Left = 672
    Top = 264
    object OD_c_compsID: TStringField
      FieldName = 'ID'
      Required = True
      Size = 1
    end
    object StringField1: TStringField
      FieldName = 'REU'
      Size = 1
    end
    object StringField2: TStringField
      FieldName = 'NAME_ORG'
      Required = True
      Size = 3
    end
    object OD_c_compsNKOM: TStringField
      FieldName = 'NKOM'
      Required = True
      Size = 3
    end
  end
  object OD_c_users: TOracleDataSet
    SQL.Strings = (
      'select s.rowid, s.id, s.cd, s.name, s.guid'
      '  from scott.t_user s'
      '  order by s.name ')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      05000000040000000400000049004400010000000000080000004E0041004D00
      4500010000000000040000004300440001000000000008000000470055004900
      4400010000000000}
    RefreshOptions = [roAfterInsert, roAfterUpdate, roAllFields]
    Session = DataModule1.OracleSession1
    DesignActivation = True
    Active = True
    AfterScroll = OD_c_usersAfterScroll
    Left = 32
    Top = 129
    object OD_c_usersID: TFloatField
      FieldName = 'ID'
      ReadOnly = True
    end
    object OD_c_usersCD: TStringField
      FieldName = 'CD'
      Required = True
      Size = 32
    end
    object OD_c_usersNAME: TStringField
      FieldName = 'NAME'
      Required = True
      Size = 50
    end
    object OD_c_usersGUID: TStringField
      FieldName = 'GUID'
      Size = 36
    end
  end
  object DS_c_users2: TDataSource
    DataSet = OD_c_users
    Left = 120
    Top = 129
  end
  object DS_user_perm2: TDataSource
    DataSet = OD_user_perm
    Left = 416
    Top = 129
  end
  object DS_s_reu_trest2: TDataSource
    DataSet = OD_s_reu_trest
    Left = 752
    Top = 161
  end
  object DS_permtp2: TDataSource
    DataSet = OD_permtp
    Left = 416
    Top = 201
  end
  object DS_t_org: TDataSource
    DataSet = OD_t_org
    Left = 752
    Top = 209
  end
  object DS_c_comps: TDataSource
    DataSet = OD_c_comps
    Left = 752
    Top = 265
  end
  object cxStyleRepository1: TcxStyleRepository
    PixelsPerInch = 96
    object cxStyle2: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = clGradientActiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
    end
  end
end
