...
uses
  ...
  Dialogs, StdCtrls, Mask, DB, OracleData, frxClass, frxDBSet, frxExportXML,
  Utils, ExtCtrls, frxExportText, frxExportCSV, StrUtils, Math, frxDesgn,
  frxExportBaseDialog, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxContainer, cxEdit, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox,
  cxImageComboBox, dxSkinsCore, dxSkinsDefaultPainters;
...
uses
  Unit_status, Unit_sel_reu, Unit_sel_usl;
...
var
  ...
  i: Double;
  isAssignedFile: Boolean;
begin
  if (cbbMg.EditValue = null) and (cbbVar.EditValue <> '5') then
  begin
...
//      exp_to_dbf(OD_data, Edit2.Text+'expkartw.dbf');
  ...
  else if (cbbVar.EditValue = '10') or (cbbVar.EditValue = '11') or (cbbVar.EditValue = '12') or (cbbVar.EditValue = '16') or (cbbVar.EditValue = '17') or (cbbVar.EditValue = '18') or (cbbVar.EditValue = '19') or (cbbVar.EditValue = '20') or (cbbVar.EditValue = '21') or (cbbVar.EditValue = '22') or (cbbVar.EditValue = '23') then // Долги для Почты, Сбербанка, для Сбербанка-2
  begin
    if RadioGroup1.ItemIndex = 0 then
    begin
    ...
    begin
      if (cbbVar.EditValue = '10') or (cbbVar.EditValue = '19') or (cbbVar.EditValue = '20') or (cbbVar.EditValue = '22') or (cbbVar.EditValue = '23') then
      begin
        OD_data.Active := false;
        ...
        begin
         ...
         // Сбер по Свободе
          ...
          begin
            ...
            Writeln(F, OD_data.FieldByName('lsk').AsString + ';' + leftstr(OD_data.FieldByName('adr').AsString, 64) + ';' + OD_data.FieldByName('summa').AsString);
         ...
         // закрыть файл
          Form_status.Close;
          if isAssignedFile = true then
          begin
            Flush(F);
            CloseFile(F);
            msg2('Долги выгружены в ' + Edit2.Text, 'Внимание', MB_OK + MB_ICONINFORMATION);
          end
          else
          begin
            msg2('Нет данных!', 'Внимание', MB_OK + MB_ICONERROR);
          end;
        end
        else
        begin
         ...
         // прочие реестры
          ...
          while not OD_data.Eof do
          begin
            Writeln(F, OD_data.FieldByName('lsk').AsString + '|' + leftstr(OD_data.FieldByName('fio').AsString, 25) + '|' + leftstr(OD_data.FieldByName('adr').AsString, 64) + '|' + OD_data.FieldByName('type').AsString + '|' + OD_data.FieldByName('type_name').AsString + '|' + OD_data.FieldByName('period').AsString + '|' + '|' + OD_data.FieldByName('summa').AsString);
            i := i + OD_data.FieldByName('summa').AsFloat;
            OD_data.Next;
          end;
         ...
         //  IntToStr(OD_data.RecordCount)+'|'+FloatToStr(RoundTo(i, -2)));
          ...
          CloseFile(F);
          Form_status.Close;
          msg2('Долги выгружены в ' + Edit2.Text + 'dolgSB.txt', 'Внимание', MB_OK + MB_ICONINFORMATION);
        end;
       ...
       //Долги для сбера, с разделителем "корп."
        ...
        begin
          if cbbVar.EditValue = '17' then
            ...
            Writeln(F, OD_data.FieldByName('lsk').AsString + '|' + leftstr(OD_data.FieldByName('fio').AsString, 25) + '|' + leftstr(OD_data.FieldByName('adr').AsString, 64) + '|' + OD_data.FieldByName('type').AsString + '|' + OD_data.FieldByName('type_name').AsString + '|' + OD_data.FieldByName('period').AsString + '||' + OD_data.FieldByName('summa').AsString + '|0');
...
//       exp_to_dbf(OD_data, Edit2.Text+'dolgP.dbf');
        Form_status.Close;
        msg2('Долги выгружены в ' + Edit2.Text + 'dolgP.dbf', 'Внимание', MB_OK + MB_ICONINFORMATION);
      end
      ...
      begin
       ...
       //Writeln(f, '=|'+ ред.29.11.2019 - отменено по требованию Сбербанка
        ...
        CloseFile(F);
        Form_status.Close;
        msg2('Долги выгружены в ' + Edit2.Text + 'dolgSB2.txt', 'Внимание', MB_OK + MB_ICONINFORMATION);
      end
      ...
      begin
       //Долги по тарифам, по кабельному тв.
        ...
        while not OD_data.Eof do
        begin
          Writeln(F, OD_data.FieldByName('lsk').AsString + '|' + leftstr(OD_data.FieldByName('fio').AsString, 25) + '|' + leftstr(OD_data.FieldByName('adr').AsString, 64) + '|' + OD_data.FieldByName('type').AsString + '|' + OD_data.FieldByName('type_name').AsString + '|' + OD_data.FieldByName('tarif_name').AsString + '|' + OD_data.FieldByName('period').AsString + '|' + '|' + OD_data.FieldByName('summa').AsString);
          i := i + OD_data.FieldByName('summa').AsFloat;
          OD_data.Next;
        end;
       //Writeln(f, '=|'+  ред.29.11.2019 - отменено по требованию Сбербанка
        ...
        CloseFile(F);
        Form_status.Close;
        msg2('Долги выгружены в ' + Edit2.Text + 'dolgSB3.txt', 'Внимание', MB_OK + MB_ICONINFORMATION);
      end;
    end;
  end
  ...
  begin
    ...
    begin
      ...
      CloseFile(F);
      Form_status.Close;
      msg2('Долги выгружены в ' + Edit2.Text + 'dolgURAL.txt', 'Внимание', MB_OK + MB_ICONINFORMATION);
    end;
  end

end;

procedure TForm_print_lists.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;
...
begin
  ...
  else if (cbbVar.EditValue = '13') or (cbbVar.EditValue = '14') then // Отчёт ТСЖ для администрации
  begin
    cbbMg.Enabled := true;
    Label1.Visible := false;
    Label3.Visible := true;
...
procedure TForm_print_lists.cxtxtValKeyPress(Sender: TObject; var Key: Char);
begin
  if RetKey(Key) then
    Key := '.';
end;
