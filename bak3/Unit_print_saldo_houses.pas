...
uses
  ...
  Dialogs, StdCtrls, DM_module1, DBCtrls, DB, OracleData,
...
begin
  ...
  //Загрузка отчета
  rep_file_:='Оборотная_по_домам1.fr3';
  try
  ...
  if OD_uslm.FieldByName('uslm').AsString = '000' then
    frxReport1.Variables['str_']:=''''+' по всем услугам '+''''
  else
    frxReport1.Variables['str_']:=''''+' по услуге: '+OD_uslm.FieldByName('nm1').AsString+'''';
  ...
  begin
    ShowMessage('Не выбран трест/РЭУ, Отмена');
    Exit;
  end;
end;
