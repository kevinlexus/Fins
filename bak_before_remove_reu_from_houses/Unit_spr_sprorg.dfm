object Form_spr_sprorg: TForm_spr_sprorg
  Left = 899
  Top = 334
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1057#1087#1088#1072#1074#1086#1095#1085#1080#1082' '#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1081
  ClientHeight = 533
  ClientWidth = 848
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox2: TGroupBox
    Left = 0
    Top = 0
    Width = 848
    Height = 57
    Align = alTop
    Caption = #1058#1080#1087' '#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1080
    TabOrder = 0
    object Label1: TLabel
      Left = 344
      Top = 24
      Width = 47
      Height = 13
      Caption = #1060#1080#1083#1100#1090#1088
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object wwDBLookupCombo1: TwwDBLookupCombo
      Left = 8
      Top = 16
      Width = 305
      Height = 21
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'NAME'#9'96'#9'NAME'#9#9)
      LookupTable = OD_org_tp
      LookupField = 'ID'
      TabOrder = 0
      AutoDropDown = False
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = False
      OnCloseUp = wwDBLookupCombo1CloseUp
    end
    object Edit1: TEdit
      Left = 397
      Top = 16
      Width = 148
      Height = 21
      TabOrder = 1
      OnChange = Edit1Change
    end
  end
  object wwDBLookupCombo2: TwwDBLookupCombo
    Left = 280
    Top = 112
    Width = 121
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'NAME'#9'96'#9'NAME'#9#9)
    DataField = 'FK_ORGTP'
    DataSource = DS_sprorg
    LookupTable = OD_org_tp2
    LookupField = 'ID'
    TabOrder = 2
    AutoDropDown = False
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = False
  end
  object wwDBLookupCombo3: TwwDBLookupCombo
    Left = 280
    Top = 144
    Width = 121
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'NAME'#9'64'#9'NAME'#9#9)
    DataField = 'PARENT_ID'
    DataSource = DS_sprorg
    LookupTable = OD_t_orgp
    LookupField = 'ID'
    TabOrder = 3
    AutoDropDown = False
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = False
  end
  object wwDBGrid1: TwwDBGrid
    Left = 0
    Top = 57
    Width = 848
    Height = 435
    ControlType.Strings = (
      'LKPAR;CustomEdit;wwDBLookupCombo4;F'
      'LKPAR2;CustomEdit;wwDBLookupCombo5;F'
      'LKTP;CustomEdit;wwDBLookupCombo6;F')
    Selected.Strings = (
      'LVL'#9'7'#9#1059#1088#1086#1074#1077#1085#1100
      'ID'#9'6'#9#1050#1086#1076'.'
      'CD'#9'10'#9#1050#1086#1076'.CD'
      'NPP'#9'6'#9#8470' '#1087'/'#1087
      'NAME'#9'26'#9#1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
      'PARENT_ID'#9'10'#9#1050#1086#1076'.~'#1088#1086#1076'.'#1086#1088#1075'.'
      'LKTP'#9'20'#9#1058#1080#1087
      'LKPAR'#9'15'#9#1056#1086#1076'.~'#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103
      'LKPAR2'#9'16'#9#1056#1086#1076'. '#1086#1088#1075'.~'#1087#1086#1089#1090#1072#1074#1097#1080#1082#1086#1074
      'REU'#9'3'#9'REU'
      'TREST'#9'6'#9'TREST'
      'BANK_CD'#9'32'#9#1050#1086#1076' '#1073#1072#1085#1082#1072'~'#1076#1083#1103' txt '#1092#1072#1081#1083'.'
      'ADR_WWW'#9'256'#9#1040#1076#1088#1077#1089' '#1089#1072#1081#1090#1072
      'EMAIL'#9'256'#9'EMAIL'
      'RASCHET_SCHET'#9'20'#9#1056'/C'
      'RASCHET_SCHET2'#9'20'#9#1056'/C-2'
      'R_SCH_ADDIT'#9'20'#9#1056'/C '#1044#1086#1087'.')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    Align = alClient
    DataSource = DS_sprorg
    Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgProportionalColResize]
    TabOrder = 1
    TitleAlignment = taLeftJustify
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    TitleLines = 2
    TitleButtons = False
    OnDblClick = wwDBGrid1DblClick
  end
  object wwDBLookupCombo4: TwwDBLookupCombo
    Left = 144
    Top = 216
    Width = 121
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'NAME'#9'64'#9'NAME'#9#9)
    DataField = 'PARENT_ID'
    DataSource = DS_sprorg
    LookupTable = OD_t_orgp
    LookupField = 'ID'
    TabOrder = 6
    AutoDropDown = False
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = True
  end
  object wwDBLookupCombo5: TwwDBLookupCombo
    Left = 144
    Top = 248
    Width = 121
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'NAME'#9'64'#9'NAME'#9#9)
    DataField = 'PARENT_ID2'
    DataSource = DS_sprorg
    LookupTable = OD_t_orgp2
    LookupField = 'ID'
    TabOrder = 7
    AutoDropDown = False
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = True
  end
  object wwDBLookupCombo6: TwwDBLookupCombo
    Left = 144
    Top = 184
    Width = 121
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'NAME'#9'96'#9'NAME'#9#9)
    DataField = 'FK_ORGTP'
    DataSource = DS_sprorg
    LookupTable = OD_org_tp2
    LookupField = 'NAME'
    TabOrder = 4
    AutoDropDown = False
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = True
  end
  object Memo1: TMemo
    Left = 480
    Top = 120
    Width = 257
    Height = 217
    Color = clRed
    Lines.Strings = (
      #1053#1072' '#1101#1090#1086#1081' '#1092#1086#1088#1084#1077' '#1087#1086#1082#1072#1079#1072#1085' '#1091#1076#1072#1095#1085#1099#1081' '#1087#1088#1080#1084#1077#1088
      #1080#1089#1087#1086#1083#1100#1079#1086#1074#1072#1085#1080#1103' '#1089#1074#1086#1081#1089#1090#1074' '#1080' '#1084#1077#1090#1086#1076#1086#1074' DOA'
      'UpdatesPending,'
      'CashedUpdates,'
      'ApplyUpdates'
      #1072' '#1090#1072#1082' '#1078#1077' '#1088#1091#1089#1089#1080#1092#1080#1094#1080#1088#1086#1074#1072#1085#1072' '#1088#1077#1072#1082#1094#1080#1103' '#1085#1072' required '
      #1087#1086#1083#1103)
    TabOrder = 5
    Visible = False
  end
  object Panel2: TPanel
    Left = 0
    Top = 492
    Width = 848
    Height = 41
    Align = alBottom
    TabOrder = 8
    DesignSize = (
      848
      41)
    object Button1: TButton
      Left = 686
      Top = 8
      Width = 75
      Height = 25
      Anchors = [akRight, akBottom]
      Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
      Enabled = False
      TabOrder = 0
      OnClick = Button1Click
    end
    object Button2: TButton
      Left = 766
      Top = 8
      Width = 75
      Height = 25
      Anchors = [akRight, akBottom]
      Cancel = True
      Caption = #1047#1072#1082#1088#1099#1090#1100
      TabOrder = 1
      OnClick = Button2Click
    end
    object Panel1: TPanel
      Left = 1
      Top = 1
      Width = 448
      Height = 39
      Align = alLeft
      TabOrder = 2
      object wwDBNavigator1: TwwDBNavigator
        Left = 1
        Top = 1
        Width = 446
        Height = 37
        AutosizeStyle = asSizeNavButtons
        DataSource = DS_sprorg
        Options = [noUseInternationalText]
        ShowHint = True
        RepeatInterval.InitialDelay = 500
        RepeatInterval.Interval = 100
        Align = alClient
        ParentShowHint = False
        object wwDBNavigator1First: TwwNavButton
          Left = 0
          Top = 0
          Width = 32
          Height = 37
          Hint = #1050' '#1087#1077#1088#1074#1086#1081' '#1079#1072#1087#1080#1089#1080
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1First'
          Enabled = False
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 0
          Style = nbsFirst
        end
        object wwDBNavigator1PriorPage: TwwNavButton
          Left = 32
          Top = 0
          Width = 32
          Height = 37
          Hint = #1053#1072#1079#1072#1076' '#1085#1072' 10 '#1079#1072#1087#1080#1089#1077#1081
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1PriorPage'
          Enabled = False
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 1
          Style = nbsPriorPage
        end
        object wwDBNavigator1Prior: TwwNavButton
          Left = 64
          Top = 0
          Width = 32
          Height = 37
          Hint = #1055#1088#1077#1076#1099#1076#1091#1097#1072#1103' '#1079#1072#1087#1080#1089#1100
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Prior'
          Enabled = False
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 2
          Style = nbsPrior
        end
        object wwDBNavigator1Next: TwwNavButton
          Left = 96
          Top = 0
          Width = 32
          Height = 37
          Hint = #1057#1083#1077#1076#1091#1102#1097#1072#1103' '#1079#1072#1087#1080#1089#1100
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Next'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 3
          Style = nbsNext
        end
        object wwDBNavigator1NextPage: TwwNavButton
          Left = 128
          Top = 0
          Width = 32
          Height = 37
          Hint = #1042#1087#1077#1088#1077#1076' '#1085#1072' 10 '#1079#1072#1087#1080#1089#1077#1081
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1NextPage'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 4
          Style = nbsNextPage
        end
        object wwDBNavigator1Edit: TwwNavButton
          Left = 160
          Top = 0
          Width = 32
          Height = 37
          Hint = #1056#1077#1076#1072#1082#1090#1080#1088#1086#1074#1072#1090#1100' '#1090#1077#1082#1091#1097#1091#1102' '#1079#1072#1087#1080#1089#1100
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Edit'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 5
          Style = nbsEdit
          Visible = False
        end
        object wwDBNavigator1Cancel: TwwNavButton
          Left = 192
          Top = 0
          Width = 32
          Height = 37
          Hint = #1054#1090#1084#1077#1085#1080#1090#1100' '#1080#1079#1084#1077#1085#1077#1085#1080#1103' '#1090#1077#1082#1091#1097#1077#1081' '#1079#1072#1087#1080#1089#1080
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Cancel'
          Enabled = False
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 6
          Style = nbsCancel
        end
        object wwDBNavigator1Refresh: TwwNavButton
          Left = 224
          Top = 0
          Width = 32
          Height = 37
          Hint = #1054#1073#1085#1086#1074#1080#1090#1100' '#1079#1072#1087#1080#1089#1080
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Refresh'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 7
          Style = nbsRefresh
        end
        object wwDBNavigator1SaveBookmark: TwwNavButton
          Left = 256
          Top = 0
          Width = 32
          Height = 37
          Hint = #1055#1086#1089#1090#1072#1074#1080#1090#1100' '#1079#1072#1082#1083#1072#1076#1082#1091' '#1085#1072' '#1090#1077#1082#1091#1097#1077#1081' '#1079#1072#1087#1080#1089#1080
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1SaveBookmark'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 8
          Style = nbsSaveBookmark
          Visible = False
        end
        object wwDBNavigator1RestoreBookmark: TwwNavButton
          Left = 288
          Top = 0
          Width = 32
          Height = 37
          Hint = #1055#1077#1088#1077#1081#1090#1080' '#1082' '#1089#1086#1093#1088#1072#1085#1105#1085#1085#1086#1081' '#1079#1072#1082#1083#1072#1076#1082#1077
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1RestoreBookmark'
          Enabled = False
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 9
          Style = nbsRestoreBookmark
          Visible = False
        end
        object wwDBNavigator1Delete: TwwNavButton
          Left = 320
          Top = 0
          Width = 32
          Height = 37
          Hint = #1059#1076#1072#1083#1080#1090#1100' '#1090#1077#1082#1091#1097#1091#1102' '#1079#1072#1087#1080#1089#1100
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Delete'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          OnClick = wwDBNavigator1DeleteClick
          Index = 10
          Style = nbsDelete
        end
        object wwDBNavigator1Post: TwwNavButton
          Left = 352
          Top = 0
          Width = 32
          Height = 37
          Hint = #1054#1090#1087#1088#1072#1074#1080#1090#1100' '#1080#1079#1084#1077#1085#1077#1085#1080#1103' '#1074' '#1073#1072#1079#1091
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Post'
          Enabled = False
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 11
          Style = nbsPost
          Visible = False
        end
        object wwDBNavigator1Last: TwwNavButton
          Left = 384
          Top = 0
          Width = 31
          Height = 37
          Hint = #1055#1077#1088#1077#1081#1090#1080' '#1082' '#1087#1086#1089#1083#1077#1076#1085#1077#1081' '#1079#1072#1087#1080#1089#1080
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Last'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 12
          Style = nbsLast
        end
        object wwDBNavigator1Insert: TwwNavButton
          Left = 415
          Top = 0
          Width = 31
          Height = 37
          Hint = #1044#1086#1073#1072#1074#1080#1090#1100' '#1085#1086#1074#1091#1102' '#1079#1072#1087#1080#1089#1100
          ImageIndex = -1
          NumGlyphs = 2
          Spacing = 4
          Transparent = False
          Caption = 'wwDBNavigator1Insert'
          DisabledTextColors.ShadeColor = clGray
          DisabledTextColors.HighlightColor = clBtnHighlight
          Index = 13
          Style = nbsInsert
        end
      end
    end
  end
  object DS_sprorg: TwwDataSource
    DataSet = OD_sprorg
    Left = 112
    Top = 80
  end
  object OD_org_tp: TOracleDataSet
    SQL.Strings = (
      'select * from ('
      'select -1 as id, '#39#1042#1089#1077#39' as name from dual'
      'union all'
      'select t.id, t.name from scott.t_org_tp t)'
      'order by nvl(id,0), name')
    Optimize = False
    SequenceField.ApplyMoment = amOnNewRecord
    QBEDefinition.QBEFieldDefs = {
      0400000002000000020000004944010000000000040000004E414D4501000000
      0000}
    Session = DataModule1.OracleSession1
    DesignActivation = True
    Left = 328
    Top = 96
  end
  object DS_org_tp: TwwDataSource
    DataSet = OD_org_tp
    Left = 360
    Top = 96
  end
  object wwRecordViewDialog1: TwwRecordViewDialog
    DataSource = DS_sprorg
    FormPosition.Left = 0
    FormPosition.Top = 0
    FormPosition.Width = 0
    FormPosition.Height = 0
    NavigatorButtons = [nbsFirst, nbsPrior, nbsNext, nbsLast, nbsInsert, nbsDelete]
    ControlOptions = []
    OKCancelOptions = [rvokAutoPostRec, rvokAutoCancelRec]
    LabelFont.Charset = DEFAULT_CHARSET
    LabelFont.Color = clWindowText
    LabelFont.Height = -11
    LabelFont.Name = 'MS Sans Serif'
    LabelFont.Style = []
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Caption = 'Record View'
    Selected.Strings = (
      'ID'#9'10'#9#1050#1086#1076' ID'#9'T'
      'CD'#9'32'#9#1050#1086#1076' CD'#9'F'
      'PARENT_ID'#9'10'#9#1050#1086#1076' '#1074#1099#1096#1077#1089#1090'.ID'#9'T'
      'NAME'#9'64'#9#1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077#9#9
      'FULL_NAME'#9'64'#9#1055#1086#1083#1085#1086#1077' '#1085#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077' '#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1080#9'F'
      'REU'#9'2'#9#1050#1086#1076' '#1056#1069#1059#9#9
      'TREST'#9'2'#9#1050#1086#1076' '#1046#1069#1054#9#9
      'ADR'#9'100'#9#1040#1076#1088#1077#1089' '#1086#1088#1075'.'#9#9
      'POST_INDX'#9'10'#9#1055#1086#1095#1090#1086#1074#1099#1081' '#1080#1085#1076#1077#1082#1089#9'F'
      'INN'#9'12'#9#1048#1053#1053#9#9
      'HEAD_NAME'#9'50'#9#1044#1086#1083#1078#1085#1086#1089#1090#1100' '#1088#1091#1082#1086#1074#1086#1076#1080#1090#1077#1083#1103#9'F'
      'MANAGER'#9'50'#9#1044#1080#1088#1077#1082#1090#1086#1088#9#9
      'BUH'#9'50'#9#1041#1091#1093#1075#1072#1083#1090#1077#1088#9#9
      'RASCHET_SCHET'#9'20'#9#1056'/C'#9#9
      'RASCHET_SCHET2'#9'20'#9#1056'/C -2 '#9'F'
      'R_SCH_ADDIT'#9'20'#9#1056'/C -'#1044#1086#1087'.'#9'F'
      'K_SCHET'#9'20'#9#1050#1086#1088#1088'.'#1089#1095'.'#9#9
      'KOD_OKONH'#9'20'#9#1054#1050#1054#1053#1061#9#9
      'KOD_OGRN'#9'20'#9#1054#1043#1056#1053#9#9
      'BIK'#9'20'#9#1041#1048#1050#9#9
      'PHONE'#9'15'#9#1058#1077#1083#1077#1092#1086#1085#9#9
      'PHONE2'#9'15'#9#1058#1077#1083#1077#1092#1086#1085'-2'#9'F'
      'KPP'#9'12'#9#1050#1055#1055#9#9
      'BANK'#9'100'#9#1056#1077#1082#1074#1080#1079#1080#1090#1099' '#1073#1072#1085#1082#1072#9#9
      'ADR_RECIP'#9'100'#9#1040#1076#1088#1077#1089' '#1087#1086#1083#1091#1095#1072#1090#1077#1083#1103#9#9
      'AUTHORIZED_DIR'#9'50'#9#1059#1087#1086#1083#1085#1086#1084#1086#1095#1077#1085#1085#1099#1081' '#1087#1088#1080#1082#1072#1079#1086#1084', '#1079#1072' '#1076#1080#1088#1077#1082#1090#1086#1088#1072#9#9
      'AUTHORIZED_BUH'#9'50'#9#1059#1087#1086#1083#1085#1086#1084#1086#1095#1077#1085#1085#1099#1081' '#1087#1088#1080#1082#1072#1079#1086#1084', '#1079#1072' '#1075#1083'.'#1073#1091#1093#1072'.'#9#9
      
        'AUTH_DIR_DOC'#9'50'#9#1044#1086#1082#1091#1084#1077#1085#1090' - '#1059#1087#1086#1083#1085#1086#1084#1086#1095#1077#1085#1085#1099#1081' '#1087#1088#1080#1082#1072#1079#1086#1084', '#1079#1072' '#1076#1080#1088#1077#1082#1090#1086#1088#1072 +
        #9#9
      
        'AUTH_BUH_DOC'#9'50'#9#1044#1086#1082#1091#1084#1077#1085#1090' - '#1059#1087#1086#1083#1085#1086#1084#1086#1095#1077#1085#1085#1099#1081' '#1087#1088#1080#1082#1072#1079#1086#1084', '#1079#1072' '#1075#1083'.'#1073#1091#1093#1072'.'#9 +
        #9
      'OKPO'#9'8'#9#1054#1050#1055#1054#9#9
      'BANK_CD'#9'32'#9#1050#1086#1076' '#1073#1072#1085#1082#1072'~'#1076#1083#1103' txt '#1092#1072#1081#1083'.'#9'F'
      'ADR_WWW'#9'256'#9#1040#1076#1088#1077#1089' '#1089#1072#1081#1090#1072#9'F'
      'EMAIL'#9'256'#9'E-MAIL'#9'F')
    NavigatorFlat = True
    Left = 48
    Top = 120
  end
  object OD_org_tp2: TOracleDataSet
    SQL.Strings = (
      'select t.* from scott.t_org_tp t'
      'order by t.name')
    Optimize = False
    SequenceField.ApplyMoment = amOnNewRecord
    QBEDefinition.QBEFieldDefs = {
      
      010000000000}
    Session = DataModule1.OracleSession1
    DesignActivation = True
    Active = True
    Left = 272
    Top = 176
    object StringField1: TStringField
      DisplayWidth = 96
      FieldName = 'NAME'
      Required = True
      Size = 32
    end
    object FloatField1: TFloatField
      DisplayWidth = 10
      FieldName = 'ID'
      Required = True
      Visible = False
    end
    object StringField2: TStringField
      DisplayWidth = 32
      FieldName = 'CD'
      Required = True
      Visible = False
      Size = 32
    end
  end
  object DS_org_tp2: TwwDataSource
    DataSet = OD_org_tp2
    Left = 304
    Top = 176
  end
  object OD_t_orgp: TOracleDataSet
    SQL.Strings = (
      'select s.id, s.name'
      '  from scott.t_org s'
      'order by s.name')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0400000002000000020000004944010000000000040000004E414D4501000000
      0000}
    Session = DataModule1.OracleSession1
    DesignActivation = True
    Active = True
    Left = 272
    Top = 208
  end
  object DS_t_orgp: TwwDataSource
    DataSet = OD_t_orgp
    Left = 304
    Top = 208
  end
  object OD_sprorg: TOracleDataSet
    SQL.Strings = (
      'select decode(level, 1, '#39' '#39', rpad('#39' '#39', level, '#39'o'#39')) as lvl,'
      '       s.id,'
      '       s.cd,'
      '       s.fk_orgtp,'
      '       s.name,'
      '       s.npp,'
      '       s.v,'
      '       s.parent_id,'
      '       s.reu,'
      '       s.trest,'
      '       s.uch,'
      '       s.adr,'
      '       s.inn,'
      '       s.manager,'
      '       s.buh,'
      '       s.raschet_schet,'
      '       s.raschet_schet2,'
      '       s.k_schet,'
      '       s.kod_okonh,'
      '       s.kod_ogrn,'
      '       s.bik,'
      '       s.phone,'
      '       s.kpp,'
      '       s.bank,'
      '       s.id_exp,'
      '       s.adr_recip,'
      '       s.authorized_dir,'
      '       s.authorized_buh,'
      '       s.auth_dir_doc,'
      '       s.auth_buh_doc,'
      '       s.okpo,'
      '       s.ver_cd,'
      '       s.full_name,'
      '       s.phone2,'
      '       s.parent_id2,'
      '       s.fk_org2,'
      '       s.bank_cd,'
      '       s.adr_www,'
      '       s.email,'
      '       s.head_name,'
      '       s.post_indx,'
      '       s.r_sch_addit,'
      '       s.rowid'
      '  from scott.t_org s'
      
        ' where (:fk_orgtp <> -1 and s.fk_orgtp = :fk_orgtp or :fk_orgtp ' +
        '= -1)'
      
        '  and (:varflt_=1 and (upper(s.name) like '#39'%'#39'||upper(:flt_)||'#39'%'#39 +
        ')'
      ' or :varflt_=0)'
      ''
      ' start with s.parent_id2 is null'
      'connect by prior s.id = s.parent_id2'
      
        '--order by level, s.name --'#1086#1088#1076#1077#1088' '#1085#1077#1083#1100#1079#1103' - '#1085#1072#1088#1091#1096#1072#1077#1090#1089#1103' '#1089#1093#1077#1084#1072' '#1076#1077#1088#1077#1074 +
        #1072)
    Optimize = False
    Variables.Data = {
      0300000003000000090000003A464B5F4F524754500300000004000000FFFFFF
      FF00000000080000003A564152464C545F030000000400000000000000000000
      00050000003A464C545F050000000000000000000000}
    OracleDictionary.UseMessageTable = True
    QBEDefinition.QBEFieldDefs = {
      
      010000000000}
    RefreshOptions = [roAfterInsert, roAfterUpdate]
    OnTranslateMessage = OD_sprorgTranslateMessage
    CommitOnPost = False
    CachedUpdates = True
    Session = DataModule1.OracleSession1
    Active = True
    BeforeInsert = OD_sprorgBeforeInsert
    BeforePost = OD_sprorgBeforePost
    Left = 80
    Top = 80
    object OD_sprorgLVL: TStringField
      DisplayLabel = #1059#1088#1086#1074#1077#1085#1100
      DisplayWidth = 7
      FieldName = 'LVL'
      Size = 4000
    end
    object OD_sprorgID: TFloatField
      DisplayLabel = #1050#1086#1076'.'
      DisplayWidth = 6
      FieldName = 'ID'
    end
    object OD_sprorgCD: TStringField
      DisplayLabel = #1050#1086#1076'.CD'
      DisplayWidth = 10
      FieldName = 'CD'
      Size = 32
    end
    object OD_sprorgNPP: TFloatField
      DisplayLabel = #8470' '#1087'/'#1087
      DisplayWidth = 6
      FieldName = 'NPP'
    end
    object OD_sprorgNAME: TStringField
      DisplayLabel = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
      DisplayWidth = 26
      FieldName = 'NAME'
      Required = True
      Size = 64
    end
    object OD_sprorgPARENT_ID: TFloatField
      DisplayLabel = #1050#1086#1076'.~'#1088#1086#1076'.'#1086#1088#1075'.'
      DisplayWidth = 10
      FieldName = 'PARENT_ID'
    end
    object OD_sprorgLKTP: TStringField
      DisplayLabel = #1058#1080#1087
      DisplayWidth = 20
      FieldKind = fkLookup
      FieldName = 'LKTP'
      LookupDataSet = OD_org_tp2
      LookupKeyFields = 'ID'
      LookupResultField = 'NAME'
      KeyFields = 'FK_ORGTP'
      Lookup = True
    end
    object OD_sprorgLKPAR2: TStringField
      DisplayLabel = #1056#1086#1076'.~'#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103
      DisplayWidth = 15
      FieldKind = fkLookup
      FieldName = 'LKPAR'
      LookupDataSet = OD_t_orgp
      LookupKeyFields = 'ID'
      LookupResultField = 'NAME'
      KeyFields = 'PARENT_ID'
      Lookup = True
    end
    object OD_sprorgLKPAR: TStringField
      DisplayLabel = #1056#1086#1076'. '#1086#1088#1075'.~'#1087#1086#1089#1090#1072#1074#1097#1080#1082#1086#1074
      DisplayWidth = 16
      FieldKind = fkLookup
      FieldName = 'LKPAR2'
      LookupDataSet = OD_t_orgp
      LookupKeyFields = 'ID'
      LookupResultField = 'NAME'
      KeyFields = 'PARENT_ID2'
      Lookup = True
    end
    object OD_sprorgREU: TStringField
      DisplayWidth = 3
      FieldName = 'REU'
      Size = 2
    end
    object OD_sprorgTREST: TStringField
      DisplayWidth = 6
      FieldName = 'TREST'
      Size = 2
    end
    object OD_sprorgBANK_CD: TStringField
      DisplayLabel = #1050#1086#1076' '#1073#1072#1085#1082#1072'~'#1076#1083#1103' txt '#1092#1072#1081#1083'.'
      DisplayWidth = 32
      FieldName = 'BANK_CD'
      Size = 32
    end
    object OD_sprorgADR_WWW: TStringField
      DisplayLabel = #1040#1076#1088#1077#1089' '#1089#1072#1081#1090#1072
      DisplayWidth = 256
      FieldName = 'ADR_WWW'
      Size = 256
    end
    object OD_sprorgEMAIL: TStringField
      DisplayWidth = 256
      FieldName = 'EMAIL'
      Size = 256
    end
    object OD_sprorgRASCHET_SCHET: TStringField
      DisplayLabel = #1056'/C'
      DisplayWidth = 20
      FieldName = 'RASCHET_SCHET'
    end
    object OD_sprorgRASCHET_SCHET2: TStringField
      DisplayLabel = #1056'/C-2'
      DisplayWidth = 20
      FieldName = 'RASCHET_SCHET2'
    end
    object OD_sprorgR_SCH_ADDIT: TStringField
      DisplayLabel = #1056'/C '#1044#1086#1087'.'
      DisplayWidth = 20
      FieldName = 'R_SCH_ADDIT'
    end
    object OD_sprorgFK_ORGTP: TFloatField
      DisplayLabel = #1058#1080#1087
      DisplayWidth = 10
      FieldName = 'FK_ORGTP'
      Required = True
      Visible = False
    end
    object OD_sprorgV: TFloatField
      DisplayWidth = 10
      FieldName = 'V'
      Visible = False
    end
    object OD_sprorgUCH: TFloatField
      DisplayWidth = 10
      FieldName = 'UCH'
      Visible = False
    end
    object OD_sprorgADR: TStringField
      DisplayWidth = 100
      FieldName = 'ADR'
      Visible = False
      Size = 100
    end
    object OD_sprorgINN: TStringField
      DisplayWidth = 12
      FieldName = 'INN'
      Visible = False
      Size = 12
    end
    object OD_sprorgMANAGER: TStringField
      DisplayWidth = 50
      FieldName = 'MANAGER'
      Visible = False
      Size = 50
    end
    object OD_sprorgBUH: TStringField
      DisplayWidth = 50
      FieldName = 'BUH'
      Visible = False
      Size = 50
    end
    object OD_sprorgK_SCHET: TStringField
      DisplayWidth = 20
      FieldName = 'K_SCHET'
      Visible = False
    end
    object OD_sprorgKOD_OKONH: TStringField
      DisplayWidth = 20
      FieldName = 'KOD_OKONH'
      Visible = False
    end
    object OD_sprorgKOD_OGRN: TStringField
      DisplayWidth = 20
      FieldName = 'KOD_OGRN'
      Visible = False
    end
    object OD_sprorgBIK: TStringField
      DisplayWidth = 20
      FieldName = 'BIK'
      Visible = False
    end
    object OD_sprorgPHONE: TStringField
      DisplayWidth = 15
      FieldName = 'PHONE'
      Visible = False
      Size = 15
    end
    object OD_sprorgKPP: TStringField
      DisplayWidth = 12
      FieldName = 'KPP'
      Visible = False
      Size = 12
    end
    object OD_sprorgBANK: TStringField
      DisplayWidth = 100
      FieldName = 'BANK'
      Visible = False
      Size = 100
    end
    object OD_sprorgID_EXP: TFloatField
      DisplayWidth = 10
      FieldName = 'ID_EXP'
      Visible = False
    end
    object OD_sprorgADR_RECIP: TStringField
      DisplayWidth = 100
      FieldName = 'ADR_RECIP'
      Visible = False
      Size = 100
    end
    object OD_sprorgAUTHORIZED_DIR: TStringField
      DisplayWidth = 50
      FieldName = 'AUTHORIZED_DIR'
      Visible = False
      Size = 50
    end
    object OD_sprorgAUTHORIZED_BUH: TStringField
      DisplayWidth = 50
      FieldName = 'AUTHORIZED_BUH'
      Visible = False
      Size = 50
    end
    object OD_sprorgAUTH_DIR_DOC: TStringField
      DisplayWidth = 50
      FieldName = 'AUTH_DIR_DOC'
      Visible = False
      Size = 50
    end
    object OD_sprorgAUTH_BUH_DOC: TStringField
      DisplayWidth = 50
      FieldName = 'AUTH_BUH_DOC'
      Visible = False
      Size = 50
    end
    object OD_sprorgOKPO: TStringField
      DisplayWidth = 8
      FieldName = 'OKPO'
      Visible = False
      Size = 8
    end
    object OD_sprorgVER_CD: TStringField
      DisplayWidth = 7
      FieldName = 'VER_CD'
      Visible = False
      Size = 1
    end
    object OD_sprorgFULL_NAME: TStringField
      DisplayWidth = 64
      FieldName = 'FULL_NAME'
      Visible = False
      Size = 64
    end
    object OD_sprorgPHONE2: TStringField
      DisplayWidth = 15
      FieldName = 'PHONE2'
      Visible = False
      Size = 15
    end
    object OD_sprorgPARENT_ID2: TFloatField
      DisplayWidth = 10
      FieldName = 'PARENT_ID2'
      Visible = False
    end
    object OD_sprorgHEAD_NAME: TStringField
      DisplayWidth = 50
      FieldName = 'HEAD_NAME'
      Visible = False
      Size = 50
    end
    object OD_sprorgPOST_INDX: TStringField
      DisplayLabel = #1055#1086#1095#1090#1086#1074#1099#1081' '#1080#1085#1076#1077#1082#1089
      DisplayWidth = 10
      FieldName = 'POST_INDX'
      Visible = False
      Size = 10
    end
  end
  object OD_t_orgp2: TOracleDataSet
    SQL.Strings = (
      'select s.id, s.name'
      '  from scott.t_org s'
      'order by s.name')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0400000002000000020000004944010000000000040000004E414D4501000000
      0000}
    Session = DataModule1.OracleSession1
    DesignActivation = True
    Active = True
    Left = 272
    Top = 240
  end
  object DS_t_orgp2: TwwDataSource
    DataSet = OD_t_orgp2
    Left = 304
    Top = 240
  end
  object Timer2: TTimer
    Interval = 300
    OnTimer = Timer2Timer
    Left = 592
    Top = 16
  end
end
