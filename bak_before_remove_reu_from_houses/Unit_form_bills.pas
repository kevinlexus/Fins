...
uses
  ...
  wwradiogroup, Wwdotdot, Wwdbcomb, syQRcode, cxGraphics, cxControls,
...
begin
  ...
  OD_data.Active:=false;
   ...
   //новый счет
   if (OD_t_org.FieldByName('FNAME_SCH').asString ='sch4.fr3') and (DataModule1.OraclePackage1.CallIntegerFunction //новый вариант счета
           ('scott.Utils.get_int_param', ['SPR_BILL_VER']) = 2) then
   begin
  ...
  if (ComboBox1.ItemIndex = 0)  then //счёт
   begin
     ...
     if (OD_t_org.FieldByName('FNAME_SCH').asString ='sch4.fr3') and (DataModule1.OraclePackage1.CallIntegerFunction //новый вариант счета
             ('scott.Utils.get_int_param', ['SPR_BILL_VER']) = 2) then
     begin
       OD_detail.SetVariable('p_mg1', DBLookupComboboxEh1.KeyValue);
       OD_detail.SetVariable('p_mg2', DBLookupComboboxEh1.KeyValue);
     end;
...
procedure TForm_print_bills.DBLookupComboboxEh7CloseUp(Sender: TObject;
  Accept: Boolean);
begin
  sel_ls_cnt;
end;
...
procedure TForm_print_bills.DBLookupComboboxEh1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
  if sel_obj_=2 then
    sel_ls_cnt;
