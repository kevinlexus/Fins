...
begin
  ...
  begin
    //Единый лицевой счет
    elsk:=lsXL.WorkBooks[1].WorkSheets[1].Cells[a, 21];
    //Узнать лиц.счет (lsk)
...
begin
  ...
  begin
    //Единый лицевой счет
    elsk:=lsXL.WorkBooks[1].WorkSheets[2].Cells[a, 3];
    //Узнать лиц.счет (lsk)
    lsk:=lsXL.WorkBooks[1].WorkSheets[2].Cells[a, 2];
    if elsk <> '' then
    begin
      //если заполнен единый л.с.
      //проставить ELSK в базу Директа
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          lskXL.WorkBooks[1].WorkSheets[1].Cells[c, 20]:= d.FieldByName('kpr').AsFloat;
          c:=c+1;
        end;  
      end
      else
