...
uses
  ...
  Oracle,
  ExtCtrls, Unit_Mainform, Mask, DBCtrls, Utils, SyncObjs, abfComponents,
  ...
  cxMaskEdit, cxSpinEdit, cxTimeEdit;
...
uses DM_module1, Unit_status_gen, Unit_sel_comps, Unit_gen_err_lst, Unit_Gthread,
  Unit_smpl_chk;
...
//  Form_status_gen.Update;
  ...
  begin
    ...
    begin
      state_:=3;
      l_err_mess:='Ошибочно содержащиеся в карточке л/c, периоды действия счетчиков, прописки-выписки';
...
begin
  Memo1.Text:='Выполняется формирование...';
//  Application.CreateForm(TForm_status_gen, Form_status_gen);
  ...
  Button3.Enabled:=true;
   ...
   else if (state_=4) or (state_=5) then //Итоговое формирование произошло с ошибкой
   begin
      //отметка об окончании формирования
      DataModule1.OraclePackage2.Session.Connected:=true;
      l_state:=DataModule1.OraclePackage2.CallIntegerFunction
