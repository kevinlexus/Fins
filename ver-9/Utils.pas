...
//  SetMenuItem(acc_, N86, 'drn86_Статистика_объемн_показ');
...
begin
    ...
    msg2('Not found column '+UpperCase(ColName), 'Внимание!', MB_OK+MB_ICONERROR);
//    Result := -1;
end;
...
begin
  ...
  dTabDef.SQL.Text:='select * from DBA_TAB_COLUMNS t where t.TABLE_NAME=upper(:p_table) and t.owner=upper(:p_schema)';
  ...
  try
    for i := 0 to dset.FieldCount - 1 do
    begin
      if dTabDef.Locate('COLUMN_NAME', UpperCase(dset.Fields[i].FieldName), []) then
        else
          msg2('Не найдено поле '+UpperCase(dset.Fields[i].FieldName)+' по таблице '+UpperCase(aTable), 'Внимание!', MB_OK);

      if dTabDef.FieldByName('DATA_TYPE').AsString='NUMBER' then
        begin
            DBF1.AddFieldDefs(dset.Fields[i].FieldName, bfNumber{так странно, было bfFloat а надо Number},
