...
{ if have_date_ = 1 then
 ...
 {     try
      ...
      //Form_olap.Button3.Visible:=true; - убрал, перенёс в отдельный if, выше
...
//    SetVariable('id_', rep_id_);
  ...
  if rep_cd_ = '94' then
  begin
   Form_olap.btn1.Visible:=true;
   Form_olap.btn2.Visible:=true;
  end
  else
  begin
   Form_olap.btn1.Visible:=false;
...
begin
  ...
  begin
    //Единый лицевой счет
    elsk:=lsXL.WorkBooks[1].WorkSheets[1].Cells[a, 20];
    //Узнать лиц.счет (lsk)
    lsk:=lsXL.WorkBooks[1].WorkSheets[1].Cells[a, 2];
    if elsk <> '' then
    begin
...
//загрузка шаблона МКД в Директ, после правки в УК
procedure TForm_tree_objects.imp_gis_MKD;
var
   houseXL: variant;
   path, houseFname: String;
   a, b: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      setNumPar(houseKlsk, '', 'Год ввода в экспл.', houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 7]);
      setNumPar(houseKlsk, '', 'Этажность', houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 8]);
      setNumPar(houseKlsk, '', 'Кол-во подземных этажей', houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 9]);
      setStrPar(houseKlsk, '', 'Кадаст.номер', houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 13]);

      //найти на другом листе подъезды этого дома, попытаться создать их, или обновить по ним информацию
      a:=3;
      while true do
      begin
        entryAdr:=houseXL.WorkBooks[1].WorkSheets[3].Cells[a, 1];
        if entryAdr ='' then
          Break;
        if houseAdr = entryAdr then
        begin
          entryNum:=houseXL.WorkBooks[1].WorkSheets[3].Cells[a, 2];
          entryDtS:=houseXL.WorkBooks[1].WorkSheets[3].Cells[a, 4];
          entryDt:=StrToDateTime(entryDtS);

          entryEt:=houseXL.WorkBooks[1].WorkSheets[3].Cells[a, 3];
          entryKlsk:=DataModule1.OraclePackage1.CallIntegerFunction
                   ('scott.p_vvod.create_vvod_by_house_klsk',
                     [houseKlsk, entryNum]);
          Form_olap.cxm1.Lines.Add('Найден klsk подъезда:'+IntToStr(entryKlsk));
      ...
      while true do
      begin
        kwAdr:=houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 1];
        if kwAdr ='' then
          Break;
        if houseAdr = kwAdr then
        begin
          flag:=true;
          kwNum:=houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 2];
          kwEntry:=houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 3];
...
var
 ...
 reu, path, lskFname, houseFname: String;
 a, b, c, e, f, g, i, entr, houseId: Integer;
 d: TOracleDataSet;
 debug: Boolean;
begin
  ...
  begin
    ...
    begin
      ...
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 5]:= d.FieldByName('house_opl').AsFloat;
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 6]:= d.FieldByName('house_opl_pasp').AsFloat;
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 7]:= d.FieldByName('house_year').AsFloat;
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 8]:= d.FieldByName('house_et').AsFloat;
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 9]:= d.FieldByName('house_unet').AsFloat;
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 11]:= d.FieldByName('clk_zone').AsString;
      houseXL.WorkBooks[1].WorkSheets[1].Cells[b, 12]:= d.FieldByName('house_cult').AsString;
    ...
    begin
      ...
      houseXL.WorkBooks[1].WorkSheets[2].Cells[e, 1]:= d.FieldByName('adr').AsString;
      houseXL.WorkBooks[1].WorkSheets[2].Cells[e, 2]:= d.FieldByName('kw').AsString;
      houseXL.WorkBooks[1].WorkSheets[2].Cells[e, 3]:= d.FieldByName('comm_use').AsString;
      houseXL.WorkBooks[1].WorkSheets[2].Cells[e, 4]:= d.FieldByName('opl').AsString;
      // НЕ выгружать кадастровый номер пока, система пишет: INT004072 Сведения в ГКН не найдены.
      houseXL.WorkBooks[1].WorkSheets[2].Cells[e, 5]:= 'Нет';
      e:=e+1;
    end;

    //Подъезды
    if (d.FieldByName('entr').AsInteger <> entr) then
    begin
      if debug then
        Form_olap.cxm1.Lines.Add('3.'+intToStr(f));
      entr:=d.FieldByName('entr').AsInteger;
      houseXL.WorkBooks[1].WorkSheets[3].Cells[f, 1]:= d.FieldByName('adr').AsString;
      houseXL.WorkBooks[1].WorkSheets[3].Cells[f, 2]:= d.FieldByName('entr').AsString;
      houseXL.WorkBooks[1].WorkSheets[3].Cells[f, 3]:= d.FieldByName('ent_et').AsString;
      houseXL.WorkBooks[1].WorkSheets[3].Cells[f, 4]:= d.FieldByName('ent_date').AsString;
      f:=f+1;
    end;
    ...
    if debug then
      ...
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 1]:= d.FieldByName('adr').AsString;
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 2]:= d.FieldByName('kw').AsString;
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 3]:= d.FieldByName('entr').AsString;
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 4]:= d.FieldByName('charact').AsString;
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 5]:= d.FieldByName('opl').AsString;
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 6]:= d.FieldByName('opl').AsString;
      // НЕ выгружать кадастровый номер пока, система пишет: INT004072 Сведения в ГКН не найдены.
      houseXL.WorkBooks[1].WorkSheets[4].Cells[a, 7]:= 'Нет';
    //Лицевой счет
    //Основные сведения
    ...
    lskXL.WorkBooks[1].WorkSheets[2].Cells[a, 4]:= d.FieldByName('kw').AsString;
    lskXL.WorkBooks[1].WorkSheets[2].Cells[a, 7]:= d.FieldByName('part').AsFloat;
