...
begin
  ...
  for i := 0 to lst1.InnerListView.Items.Count - 1 do
    ...
    begin
      ...
      fName := UpperCaseW(ExtractFileName(fNamePath));
      frmMain.Memo1.Lines.Add('Попытка загрузить: '+fNamePath);
      with frmMain.OD_upd_obj do
      begin
        Active:=False;
        if frmMain.OD_du_obj.Locate('CD', fName, []) then
        begin
        ...
        begin
          SQL.Text:='insert into scott.DU_OBJ t (cd, obj, dt, sz)'+
                'values(:cd, :obj, :dt, :sz)';
        end;
