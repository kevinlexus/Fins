object Form_sch_history: TForm_sch_history
  Left = 283
  Top = 210
  Width = 811
  Height = 434
  Caption = #1048#1089#1090#1086#1088#1080#1103' '#1074#1074#1086#1076#1072' '#1087#1086#1082#1072#1079#1072#1085#1080#1081' '#1089#1095#1077#1090#1095#1080#1082#1086#1074
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object pgc1: TPageControl
    Left = 0
    Top = 0
    Width = 795
    Height = 396
    ActivePage = ts2
    Align = alClient
    TabOrder = 0
    object ts1: TTabSheet
      Caption = #1048#1089#1090#1086#1088#1080#1103' '#1089#1095#1077#1090#1095#1080#1082#1086#1074
      object pnl1: TPanel
        Left = 0
        Top = 0
        Width = 787
        Height = 25
        Align = alTop
        TabOrder = 0
        object Label1: TLabel
          Left = 8
          Top = 9
          Width = 46
          Height = 13
          Caption = #1060#1080#1083#1100#1090#1088':'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object wwDBLookupCombo1: TwwDBLookupCombo
          Left = 56
          Top = 2
          Width = 161
          Height = 21
          DropDownAlignment = taLeftJustify
          Selected.Strings = (
            'NAME'#9'25'#9'NAME'#9'F')
          LookupTable = OD_tp
          LookupField = 'ID'
          TabOrder = 0
          AutoDropDown = False
          ShowButton = True
          PreciseEditRegion = False
          AllowClearKey = False
          OnCloseUp = wwDBLookupCombo1CloseUp
        end
      end
      object cxGrid1: TcxGrid
        Left = 0
        Top = 25
        Width = 787
        Height = 343
        Align = alClient
        TabOrder = 1
        object cxGridDBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = DS_data
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          FilterRow.InfoText = #1050#1083#1080#1082' '#1079#1076#1077#1089#1100' '#1076#1083#1103' '#1086#1087#1088#1077#1076#1077#1083#1077#1085#1080#1103' '#1092#1080#1083#1100#1090#1088#1072
          FilterRow.Visible = True
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.Navigator = True
          OptionsView.GroupByBox = False
          OptionsView.HeaderAutoHeight = True
          object cxGridDBTableView1NAME_USL: TcxGridDBColumn
            Caption = #1059#1089#1083#1091#1075#1072
            DataBinding.FieldName = 'NAME_USL'
          end
          object cxGridDBTableView1NAME_ACT: TcxGridDBColumn
            Caption = #1058#1080#1087' '#1074#1074#1086#1076#1072
            DataBinding.FieldName = 'NAME_ACT'
            Width = 167
          end
          object cxGridDBTableView1NAME_TP: TcxGridDBColumn
            Caption = #1048#1089#1090#1086#1095#1085#1080#1082' '#1074#1074#1086#1076#1072
            DataBinding.FieldName = 'NAME_TP'
          end
          object cxGridDBTableView1NAME_USER: TcxGridDBColumn
            Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1100
            DataBinding.FieldName = 'NAME_USER'
            Width = 118
          end
          object cxGridDBTableView1VOL: TcxGridDBColumn
            Caption = #1056#1072#1089#1093#1086#1076'/'#1055#1086#1082#1072#1079#1072#1085#1080#1077
            DataBinding.FieldName = 'VOL'
          end
          object cxGridDBTableView1TS: TcxGridDBColumn
            Caption = #1044#1072#1090#1072'-'#1074#1088#1077#1084#1103
            DataBinding.FieldName = 'TS'
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridDBTableView1
        end
      end
    end
    object ts2: TTabSheet
      Caption = #1044#1086#1083#1080' '#1086#1073#1098#1077#1084#1086#1074
      ImageIndex = 1
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 787
        Height = 49
        Align = alTop
        TabOrder = 0
        object lbl1: TLabel
          Left = 8
          Top = 10
          Width = 114
          Height = 13
          Caption = #1058#1080#1087' '#1087#1088#1077#1076#1089#1090#1072#1074#1083#1077#1085#1080#1103
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbl2: TLabel
          Left = 80
          Top = 34
          Width = 41
          Height = 13
          Caption = #1059#1089#1083#1091#1075#1072
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object cbb2: TcxLookupComboBox
          Left = 128
          Top = 26
          Properties.KeyFieldNames = 'USL'
          Properties.ListColumns = <
            item
              FieldName = 'NM'
            end>
          Properties.ListSource = DS_usl
          Properties.OnCloseUp = cbb2PropertiesCloseUp
          TabOrder = 0
          Width = 353
        end
        object cbb1: TcxLookupComboBox
          Left = 128
          Top = 4
          Properties.KeyFieldNames = 'ID'
          Properties.ListColumns = <
            item
              FieldName = 'NAME'
            end>
          Properties.ListSource = DS_tp2
          Properties.OnCloseUp = cbb1PropertiesCloseUp
          TabOrder = 1
          Width = 353
        end
      end
      object cxgrd1: TcxGrid
        Left = 0
        Top = 49
        Width = 787
        Height = 319
        Align = alClient
        TabOrder = 1
        object cxgrdbtblvwGrid1DBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = DS_c_charge_prep
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          FilterRow.InfoText = #1050#1083#1080#1082' '#1079#1076#1077#1089#1100' '#1076#1083#1103' '#1086#1087#1088#1077#1076#1077#1083#1077#1085#1080#1103' '#1092#1080#1083#1100#1090#1088#1072
          FilterRow.Visible = True
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.Navigator = True
          OptionsView.GroupByBox = False
          OptionsView.HeaderAutoHeight = True
          object cxgrdbtblvwGrid1DBTableView1USL: TcxGridDBColumn
            Caption = #1050#1086#1076' '#1091#1089#1083'.'
            DataBinding.FieldName = 'USL'
            Width = 44
          end
          object cxgrdbtblvwGrid1DBTableView1NM: TcxGridDBColumn
            Caption = #1059#1089#1083#1091#1075#1072
            DataBinding.FieldName = 'NM'
          end
          object cxgrdbtblvwGrid1DBTableView1VOL: TcxGridDBColumn
            Caption = #1054#1073#1098#1077#1084' '#1080#1090#1086#1075'.'
            DataBinding.FieldName = 'VOL'
          end
          object cxgrdbtblvwGrid1DBTableView1VOL_NRM: TcxGridDBColumn
            Caption = #1054#1073#1098#1077#1084' '#1085#1086#1088#1084#1072#1090'.'
            DataBinding.FieldName = 'VOL_NRM'
          end
          object cxgrdbtblvwGrid1DBTableView1VOL_SV_NRM: TcxGridDBColumn
            Caption = #1054#1073#1098#1077#1084' '#1089#1074'.'#1085#1086#1088#1084'.'
            DataBinding.FieldName = 'VOL_SV_NRM'
          end
          object cxgrdbtblvwGrid1DBTableView1KPR: TcxGridDBColumn
            Caption = #1055#1088#1086#1078#1080#1074#1072#1102#1097#1080#1093
            DataBinding.FieldName = 'KPR'
          end
          object cxgrdbtblvwGrid1DBTableView1KPRZ: TcxGridDBColumn
            Caption = #1042#1088#1077#1084'.'#1079#1072#1088#1077#1075#1080#1089#1090#1088'.'
            DataBinding.FieldName = 'KPRZ'
          end
          object cxgrdbtblvwGrid1DBTableView1KPRO: TcxGridDBColumn
            Caption = #1042#1088#1077#1084' '#1086#1090#1089#1091#1090#1089#1090#1074'.'
            DataBinding.FieldName = 'KPRO'
          end
          object cxgrdbtblvwGrid1DBTableView1SCH: TcxGridDBColumn
            Caption = #1053#1072#1083#1080#1095#1080#1077' '#1089#1095#1077#1090#1080#1082#1072
            DataBinding.FieldName = 'SCH'
            PropertiesClassName = 'TcxCheckBoxProperties'
            Properties.ValueChecked = 1
            Properties.ValueUnchecked = 0
          end
          object cxgrdbtblvwGrid1DBTableView1DT: TcxGridDBColumn
            Caption = #1044#1072#1090#1072
            DataBinding.FieldName = 'DT'
          end
        end
        object cxgrd1Level1: TcxGridLevel
          GridView = cxgrdbtblvwGrid1DBTableView1
        end
      end
    end
  end
  object OD_data: TOracleDataSet
    SQL.Strings = (
      'select '
      
        '      decode(t.tp, 0, '#39#1056#1091#1095#1085#1086#1081' '#1074#1074#1086#1076#39', 1, '#39#1040#1074#1090#1086#1085#1072#1095#1080#1089#1083#1077#1085#1080#1077#39', 2, '#39#1054#1090 +
        #1084#1077#1085#1072' '#1072#1074#1090#1086#1085#1072#1095'.'#39') AS name_tp,'
      '      s.name name_act,'
      '      t.ts, us.name as name_user,'
      '      t.n1 as vol,'
      '      u.nm as name_usl'
      
        '  from scott.t_objxpar t, scott.u_list s, scott.t_user us, scott' +
        '.u_listtp tp, scott.usl u'
      ' where fk_lsk = :lsk'
      '   and t.fk_list = s.id'
      '   and t.fk_user = us.id(+)'
      
        '   and t.tp <> 3 --'#1082#1088#1086#1084#1077' '#1086#1095#1080#1089#1090#1082#1080' '#1089#1095#1077#1090#1095#1080#1082#1086#1074' '#1074' '#1087#1077#1088#1077#1093#1086#1076' - '#1079#1072#1075#1088#1103#1079#1085#1103#1077 +
        #1090' '#1074#1080#1076' '#1080#1085#1072#1095#1077
      '--   and s.cd in ('#39'ins_sch'#39','#39'ins_vol_sch'#39')'
      '   and (:var=-1 or :var=s.id)'
      '   and s.fk_listtp=tp.id '
      '   and tp.cd='#39#1055#1072#1088#1072#1084#1077#1090#1088#1099' '#1083#1080#1094'.'#1089#1095#1077#1090#1072#39
      '   and t.fk_usl=u.usl'
      ' order by t.id desc')
    Optimize = False
    Variables.Data = {
      0300000002000000040000003A4C534B05000000090000002020202020202020
      0000000000040000003A564152030000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      0400000006000000070000004E414D455F545001000000000002000000545301
      0000000000090000004E414D455F55534552010000000000080000004E414D45
      5F414354010000000000080000004E414D455F55534C01000000000003000000
      564F4C010000000000}
    Master = Form_list_kart.OD_list_kart
    MasterFields = 'LSK'
    DetailFields = 'LSK'
    Session = DataModule1.OracleSession1
    DesignActivation = True
    Active = True
    Left = 8
    Top = 144
  end
  object DS_data: TDataSource
    DataSet = OD_data
    Left = 48
    Top = 144
  end
  object OD_tp: TOracleDataSet
    SQL.Strings = (
      'select -1 as npp, null as cd, -1 as id, '#39#1042#1089#1077#39' as name from dual'
      'union all'
      'select u.npp, u.cd, u.id, u.name from scott.u_list u'
      ' where u.cd in ('#39'ins_sch'#39','#39'ins_vol_sch'#39')'
      'order by npp')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0400000004000000030000004E50500100000000000200000049440100000000
      00040000004E414D45010000000000020000004344010000000000}
    Session = DataModule1.OracleSession1
    Active = True
    Left = 8
    Top = 192
  end
  object OD_c_charge_prep: TOracleDataSet
    SQL.Strings = (
      
        'select u.usl, u.nm, t.vol, t.vol_nrm, t.vol_sv_nrm, t.kpr, t.kpr' +
        'z, t.kpro, t.sch, t.dt '
      ' from scott.c_charge_prep t, scott.usl u'
      'where t.usl=u.usl and t.lsk=:lsk'
      'and (:usl='#39'000'#39' or :usl<>'#39'000'#39' and u.usl=:usl)'
      'and t.tp=:tp'
      'order by u.npp, t.dt')
    Optimize = False
    Variables.Data = {
      0300000003000000040000003A55534C05000000000000000000000003000000
      3A5450030000000000000000000000040000003A4C534B050000000900000020
      202020202020200000000000}
    QBEDefinition.QBEFieldDefs = {
      
      44540100000000000300000055534C010000000000}
    Master = Form_list_kart.OD_list_kart
    MasterFields = 'LSK'
    DetailFields = 'LSK'
    Session = DataModule1.OracleSession1
    Active = True
    Left = 12
    Top = 256
  end
  object DS_c_charge_prep: TDataSource
    DataSet = OD_c_charge_prep
    Left = 56
    Top = 256
  end
  object OD_usl: TOracleDataSet
    SQL.Strings = (
      'select -1 as npp, '#39'000'#39' as usl,  '#39#1042#1089#1077#39' as nm from dual'
      'union all'
      'select u.npp, u.usl, u.nm from scott.usl u'
      'order by npp')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0400000003000000020000004E4D0100000000000300000055534C0100000000
      00030000004E5050010000000000}
    Session = DataModule1.OracleSession1
    Active = True
    Left = 12
    Top = 312
  end
  object DS_usl: TDataSource
    DataSet = OD_usl
    Left = 56
    Top = 312
  end
  object OD_tp2: TOracleDataSet
    SQL.Strings = (
      
        'select 0 as npp, 1 as id, '#39'1 - '#1089#1075#1088#1091#1087#1087#1080#1088#1086#1074#1072#1085#1085#1099#1081' '#1076#1086' '#1082#1083#1102#1095#1077#1074#1099#1093' '#1089#1090#1088#1091#1082 +
        #1090#1091#1088#39' as name from dual'
      'union all'
      
        'select 1 as npp, 2 as id, '#39'2 - '#1082#1086#1083'-'#1074#1086' '#1087#1088#1086#1078'.'#1076#1083#1103' '#1086#1087#1088#1077#1076'.'#1089#1086#1094#1085#1086#1088#1084#1099#39' a' +
        's name from dual'
      'union all'
      
        'select 2 as npp, 3 as id, '#39'3 - '#1080#1090#1086#1075' '#1086#1073#1098#1105#1084#1072', '#1087#1088#1086#1078#1080#1074#1072#1102#1097#1080#1093#39' as name' +
        ' from dual'
      'union all'
      
        'select 3 as npp, 4 as id, '#39'4 - '#1082#1086#1088#1088#1077#1082#1090#1080#1088#1086#1074#1082#1080' '#1054#1044#1053#39' as name from d' +
        'ual'
      'union all'
      
        'select 4 as npp, 5 as id, '#39'5 - '#1076#1077#1090#1072#1083#1100#1085#1086', '#1085#1086' '#1073#1077#1079' '#1082#1086#1088#1088#1077#1082#1090#1080#1088#1086#1074#1086#1082' '#1054#1044 +
        #1053#39' as name from dual'
      'union all'
      
        'select 5 as npp, 6 as id, '#39'6 - '#1080#1090#1086#1075' '#1086#1073#1098#1105#1084#1072', '#1087#1088#1086#1078#1080#1074#1072#1102#1097#1080#1093' '#1073#1077#1079' '#1082#1086#1088#1088 +
        #1077#1082#1090'. '#1054#1044#1053#39' as name from dual'
      'union all'
      
        'select 6 as npp, 7 as id, '#39'7 - '#1085#1072#1083#1080#1095#1080#1077' '#1089#1095#1077#1090#1095#1080#1082#1072' '#1074' '#1090#1077#1082#1091#1097#1077#1084' '#1087#1077#1088#1080#1086#1076 +
        #1077#39' as name from dual'
      'union all'
      
        'select 7 as npp, 0 as id, '#39'0 - '#1087#1086#1083#1085#1072#1103' '#1076#1077#1090#1072#1083#1080#1079#1072#1094#1080#1103' ('#1086#1073#1099#1095#1085#1086' '#1089#1082#1088#1099#1090' ' +
        #1074' '#1087#1072#1082#1077#1090#1077')'#39' as name from dual'
      'order by npp')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0400000003000000020000004944010000000000040000004E414D4501000000
      0000030000004E5050010000000000}
    Session = DataModule1.OracleSession1
    Active = True
    Left = 116
    Top = 312
  end
  object DS_tp2: TDataSource
    DataSet = OD_tp2
    Left = 160
    Top = 312
  end
end
