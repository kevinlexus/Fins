...
begin
  ...
  if (DBEditEh1.Text = '') and (Form_requests.access_type_ = 0) then
    begin
      msg2('Не заполнен адрес!','Внимание',MB_OK+MB_ICONSTOP);
      Abort;
    end;
  if (wwDBEdit4.Text = '') and (Form_requests.access_type_ = 1) then
    begin
      msg2('Не заполнено примечание!','Внимание',MB_OK+MB_ICONSTOP);
      Abort;
    end;
...
begin
{  if Application.MessageBox('Удалить работу?',
...
{ PageControl1.ActivePageIndex:=0;
  ...
  if Form_requests.access_type_ = 0  then //Заявки
  begin
  ...
  begin
  //новый документ
  //разрешено ли заполнять вкладки основное, дополнительное?
  ...
  //разрешено ли заполнять работы (обработку документа)?
   if DataModule1.OraclePackage1.CallIntegerFunction(
  ...
  //Разрешение на доступ к вкладкам в зависимости от типа приложения
  if Form_requests.access_type_ = 0  then //Заявки
  begin
  ...
  else if Form_requests.access_type_ = 1 then //Журнал пользователей
  begin
