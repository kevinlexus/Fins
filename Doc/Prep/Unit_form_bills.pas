...
uses
  ...
  frxExportXLS, frxExportRTF, Grids, DBGrids, wwclearbuttongroup,
  wwradiogroup, Wwdotdot, Wwdbcomb, syQRcode;
...
type
  TForm_print_bills = class(TForm)
    ...
    procedure DBLookupComboboxEh6DropDown(Sender: TObject);
    procedure DBLookupComboboxEh5CloseUp(Sender: TObject; Accept: Boolean);
    procedure DBLookupComboboxEh7DropDown(Sender: TObject);
    procedure sel_lsk;
  private
    sel_obj_: Integer;
    cnt_sch_: Integer;
...
begin
  ...
  OD_main.SetVariable('reu_', DBLookupComboboxEh7.KeyValue);
  ...
  OD_main.SetVariable('kw_', DBLookupComboboxEh4.KeyValue);

  if sel_obj_=2 then
  begin
    //только для УК
    //ограничивать диапазон записи для печати счетов
  //  OD_ls_cnt.Locate('lvl', wwDBLookupCombo2.LookupValue, []);
    OD_main.SetVariable('cnt_rec_', StrToInt(DBLookupComboboxEh6.KeyValue));
  ...
  if (ComboBox1.ItemIndex = 0)  then //счёт
   ...
   begin
     ...
     OD_arch.SetVariable('mg1_', DBLookupComboboxEh1.KeyValue);
     OD_arch.SetVariable('mg2_', DBLookupComboboxEh5.KeyValue);
     ...
     //по л.с.
      ...
      OD_kart.SetVariable('adr_', 0);
     end
     else
     begin
     //по адресу
      OD_kart.SetVariable('lsk', OD_kw.FieldByName('lsk').AsString);
      OD_kart.SetVariable('adr_', 1);
     end;
      OD_kart.Active:=true;
   end
  else if
    (ComboBox1.ItemIndex = 5) then //справка арх-2
   begin
     tp_:=5;
     wwDBEdit2.Text:=wwDBEdit1.Text;
     OD_arch.SetVariable('mg1_', DBLookupComboboxEh1.KeyValue);
   ...
   if (tp_<>2) and (tp_<>5) and (sel_obj_ = 0) and
       (wwDBEdit1.Text = wwDBEdit2.Text) and (CheckBox3.Checked=True) then
     begin
      //по 1 лс.
          DataModule1.OraclePackage1.CallProcedure
         ('scott.GEN.prepare_arch_k_lsk', [Form_main.k_lsk_id_, pen_last_month_, 0]);
     end
   else if (tp_<>2) and (tp_<>5) and (sel_obj_ = 1) and (DBLookupComboboxEh4.KeyValue <> null)
...
//      OD_data.SetVariable('kw_', null);
    ...
    else if (tp_=2) or (tp_=5) then //справка арх
     begin
      OD_arch.SetVariable('lsk_',  wwDBEdit1.Text);
      OD_arch.SetVariable('adr_',  0);
...
//      OD_data.SetVariable('kw_', DBLookupComboboxEh4.KeyValue);
     ...
     begin
      OD_arch.SetVariable('k_lsk_id_', Form_main.k_lsk_id_);
      OD_arch.SetVariable('adr_',  1);
 ...
 //    OD_main.SetVariable('reu_', wwDBLookupCombo3.LookupValue);
   end;
    ...
    if (((tp_=0) or
      (tp_=4)) and (OD_main.RecordCount =0)
     or (tp_=1) and (OD_main.RecordCount =0)
     or (tp_=2) and (OD_arch.RecordCount =0)
     or (tp_=5) and (OD_arch.RecordCount =0)
     ) then
    begin
    ...
    begin
     ...
     if (tp_=0) or (tp_=4) then
      begin
        //Счета
        frxReport1.LoadFromFile(Form_main.exepath_+OD_t_org.FieldByName('FNAME_SCH').asString, True);
        Edit2.Text:='Путь к файлу счета:'+Form_main.exepath_+OD_t_org.FieldByName('FNAME_SCH').asString;
        Edit2.Visible:=true;
...
if (ComboBox1.ItemIndex = 2) or (ComboBox1.ItemIndex = 5) then // Справка из архива
begin
...
if ComboBox1.ItemIndex = 3 then // Справка о задолженности
begin
  Label3.Caption:='Период отчета';
  Label9.Visible:=false;
  DBLookupComboboxEh5.Visible:=false;
...
//по л.с.
  Form_main.c_lsk_id_:=DataModule1.OraclePackage1.CallIntegerFunction
   ('scott.UTILS.GET_C_LSK_ID_BY_LSK', [lsk1_]);
  Form_main.k_lsk_id_:=DataModule1.OraclePackage1.CallIntegerFunction
   ('scott.UTILS.GET_K_LSK_ID_BY_LSK', [lsk1_]);
end
else
begin
//по адресу
...
begin
  ...
  if DataModule1.OraclePackage1.CallIntegerFunction
         ('scott.Utils.get_int_param', ['RECHARGE_BILL']) = 1 then
...
begin
  ...
  //по УК
    //кол-во л.с. для печати
    DBLookupComboboxEh6.Enabled:=true;
...
begin
  ...
  OD_ls_cnt.SetVariable('mg_', DBLookupComboboxEh1.KeyValue);
  OD_ls_cnt.Active:=true;
  DBLookupComboboxEh6.KeyValue:=OD_ls_cnt.FieldByName('lvl').AsString;
...
procedure TForm_print_bills.DBLookupComboboxEh6DropDown(Sender: TObject);
begin
  if OD_ls_cnt.GetVariable('cnt_') <> IntToStr(cnt_sch_) then
  begin
    OD_ls_cnt.SetVariable('cnt_', cnt_sch_);
    OD_ls_cnt.Active:=False;
    OD_ls_cnt.Active:=True;
  end;
end;

procedure TForm_print_bills.DBLookupComboboxEh5CloseUp(Sender: TObject;
  Accept: Boolean);
begin
  sel_ls_cnt;
end;

procedure TForm_print_bills.DBLookupComboboxEh7DropDown(Sender: TObject);
begin
  sel_ls_cnt;
end;
