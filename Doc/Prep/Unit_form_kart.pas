...
uses
  ...
  Dialogs, StdCtrls, Mask, DBCtrls, DB, OracleData, Oracle, DBCtrlsEh, DBLookupEh,
  ExtCtrls, Grids, DBGridEh, Buttons, wwcheckbox, wwSpeedButton,
  wwDBNavigator, wwclearpanel, wwdbedit, ComCtrls, Utils,
  wwfltdlg, Wwdbigrd, Wwdbgrid, GridsEh, 
  wwdbdatetimepicker, wwdblook, Wwdotdot, frxClass, frxDBSet,
  MemTableDataEh, MemTableEh, DataDriverEh, Menus, DBGrids, Wwdbcomb,
...
type  
  TForm_kart = class(TForm)
    ...
    procedure DBGridEh2Columns12EditButtonDown(Sender: TObject;
    ...
    procedure DBLookupComboboxEh13CloseUp(Sender: TObject;
      Accept: Boolean);
    procedure DBLookupComboboxEh16CloseUp(Sender: TObject;
      Accept: Boolean);
...
implementation
uses Unit_form_kart_pr, Unit_form_subsidii, Unit_Mainform, Unit_list_kart,
  Unit_new_lsk, DM_module1, Unit_chargepay, Unit_det_chrg,
  Unit_log_actions, Unit_sel_lsk_contr, Unit_set_krt_psch,
  Unit_change_house_nabor2, Unit_sch_history, Unit_reg_sch;
type
...
//  LockControl(DBGridEh1, true);
 ...
 if (Form_list_kart.OD_list_kart.UpdateStatus in [usInserted, usModified, usDeleted])
  ...
  or (Form_kart.OD_states_sch.UpdatesPending = true)
  or (updates_ = 1) then
begin
  ...
  if ask_ = 0 then //записываем без вопросов
   begin
    ...
    err_:=DataModule1.OraclePackage1.CallStringFunction(
     'scott.UTILS.tst_krt', [Form_list_kart.OD_list_kart.FieldByName('lsk').AsString, 1]);
   ...
   begin
    ...
    begin
     if (Msg3('Сохранить измения карточки?',
     'Подтверждение', MB_ICONQUESTION+MB_YESNO) = IDYES) then // сохраняем с вопросом
     begin
      //для подтверждения изменений, сделанных в пакетах
      DataModule1.OracleSession1.Commit;
      DataModule1.OracleSession1.ApplyUpdates([Form_kart.OD_charge], true);
...
LockWindowUpdate(handle);
  ...
  err_:=DataModule1.OraclePackage1.CallStringFunction(
   'scott.UTILS.tst_krt', [Form_list_kart.OD_list_kart.FieldByName('lsk').AsString, 0]);

  if DataModule1.OraclePackage1.CallIntegerFunction
       ('scott.Utils.get_int_param', ['SHOW_ALERT1']) = 1 then
  begin
    if err_ <> '' then
    begin
...
//    Form_list_kart.OD_list_kart.FieldByName('n_nd').AsString;
  ...
  //Закрытый Л/С
  if (Form_list_kart.OD_list_kart.FieldByName('psch').AsInteger = 9)
   or (Form_list_kart.OD_list_kart.FieldByName('psch').AsInteger = 8) then
  begin
    Label47.Caption:='Счет закрыт';
    Label47.Font.Color:=clRed;
  end
  else
  begin
    Label47.Caption:='Счет открыт';
...
//разрешено ли править поле показания счетчика
if (DataModule1.OraclePackage1.CallIntegerFunction
       ('scott.INIT.get_is_cnt_sch', [parNone]) = 1)
    or (DataModule1.OraclePackage1.CallIntegerFunction
         ('scott.INIT.get_is_cnt_sch', [parNone]) = 0){ and   --бред какой то (попытка запретить ввести показания не через ЛК?) 
...
begin
  Active:=false;
  SetVariable('reu',
    Form_list_kart.OD_list_kart.FieldByName('reu').AsString);
  SetVariable('kul',
     OD_spul.FieldByName('id').AsString);
  Active:=true;
end;
...
//    begin
      cnt_:=DataModule1.OraclePackage1.CallIntegerFunction(
       'scott.C_CHARGES.gen_charges', [Form_list_kart.OD_list_kart.FieldByName('lsk').AsString,
       null, null, null, 1, 0]);
...
begin
  ...
  begin  // из текущего в архив
      ...
      change_alias(OD_charge,'scott.c_charge',
        '(select * from scott.a_charge where mg='''+Form_main.arch_mg_+''')');
      change_alias(OD_charge,'scott.c_change',
        '(select * from scott.a_change where mg='''+Form_main.arch_mg_+''')');
      change_alias(OD_charge,'scott.nabor_progs',
        '(select * from scott.a_nabor_progs where mg='''+Form_main.arch_mg_+''')');
  ...
  begin  // из архива в текущее
      change_alias(OD_kart_pr,
        '(select * from scott.a_kart_pr where mg='''+mgold_+''')',
        'scott.c_kart_pr');
      change_alias(OD_kart_pr,
      ...
      change_alias(OD_vvod,
        ...
        'scott.c_vvod');
      change_alias(OD_charge,
        '(select * from scott.a_vvod where mg='''+mgold_+''')',
        'scott.c_vvod');
      change_alias(OD_charge,
        '(select * from scott.a_charge where mg='''+mgold_+''')',
        'scott.c_charge');
      change_alias(OD_charge,
        '(select * from scott.a_change where mg='''+mgold_+''')',
        'scott.c_change');
      change_alias(OD_charge,
      '(select * from scott.a_nabor_progs where mg='''+mgold_+''')',
      'scott.nabor_progs');
      change_alias(OD_charge,
...
begin
  ...
  //Включение возможности Админ-опции правки признака счетчика и т.п.
   ...
   if (DataModule1.OraclePackage1.CallIntegerFunction
          ('scott.init.is_allow_acc', ['drx5_админ_доступ_к_базе'])=0) then
   begin
...
{   try
  ...
  if (DataModule1.OraclePackage1.CallIntegerFunction
            ('scott.init.is_allow_acc', ['drx3_Правка_кода_РЭУ'])=0) then
...
{   try
  ...
  if (DataModule1.OraclePackage1.CallIntegerFunction
            ('scott.init.is_allow_acc', ['drx4_Правка_кода_Пасп_стола'])=0) then
      DBLookupComboboxEh5.Enabled:=false;
...
{   try
  ...
  //организации ЖКХ
     ...
     Height:=PageControl3.Height+Panel2.Height+Panel4.Height+
       PageControl2.Height+200;
     OD_spr_tarif.Active:=False;
     Mem_spr_tarif.Active:=False;
//     OD_vvod.Active:=true;
     OD_status.Active:=True;
     //скрыть внешний лицевой для Энерг+
  ...
  //Энерг +
     ...
     Height:=PageControl3.Height+Panel4.Height+
       PageControl2.Height+40;
  ...
  if DataModule1.OraclePackage1.CallIntegerFunction
         ('scott.INIT.get_kart_ed1', [parNone]) = 1 then
  begin
...
procedure TForm_kart.DBCheckBoxEh1MouseDown(Sender: TObject;
...
procedure TForm_kart.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then
    open_kart_pr;
...
procedure TForm_kart.wwDBLookupCombo2CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  OD_charge.FieldByName('FK_TARIF').AsInteger:=
    OD_tarif.FieldByName('ID').AsInteger;
end;
...
procedure TForm_kart.DBLookupComboboxEh13CloseUp(Sender: TObject;
  Accept: Boolean);
begin
  changes_sch_el(0);
end;

procedure TForm_kart.DBLookupComboboxEh16CloseUp(Sender: TObject;
  Accept: Boolean);
begin
  changes_sch_el(1);
end;

procedure TForm_kart.wwDBLookupCombo3CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  OD_charge.FieldByName('FK_VVOD').AsInteger:=
    OD_vvod.FieldByName('ID').AsInteger;
end;

procedure TForm_kart.wwDBLookupCombo3NotInList(Sender: TObject;
  LookupTable: TDataSet; NewValue: String; var Accept: Boolean);
begin
  msg2('Данный ввод не найден!', 'Ошибка', MB_OK+MB_ICONSTOP);
  OD_charge.FieldByName('FK_VVOD').AsVariant:=
    null;
...
begin
  ...
  if wwDBLookupCombo3.Text='' then
    OD_charge.FieldByName('FK_VVOD').AsVariant:=
      null
  else if OD_vvod.SearchRecord('VVOD_NUM', StrToInt(wwDBLookupCombo3.Text),
...
procedure TForm_kart.DBGridEh2DragOver(Sender, Source: TObject; X,
...
procedure TForm_kart.DBGridEh3MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TForm_kart.DBGridEh2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TForm_kart.DBGridEh2DragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
  cnt_: Integer;
  bm: TBookmark;
...
procedure TForm_kart.DBGridEh3DragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
  bm: TBookmark;
  cnt_: Integer;
...
procedure TForm_kart.DBGridEh3DragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin
  //Разрешить/запретить правку
  if allow_dtv_=1 then
...
begin
  ...
  begin
  msg2('Данный признак счёта запрещено устанавливать!', 'Внимание!', MB_OK+MB_ICONSTOP);
  OD_psch.Locate('ID', 0, []);
  end
  else
  begin
...
begin
  if OD_psch.SearchRecord('NAME', wwDBLookupCombo4.Text,
    [srFromBeginning]) <> true then
  begin
...
procedure TForm_kart.wwDBGrid4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then
    open_kart_pr;
...
begin
  str_:='';
  if msg3('Удалить данный лицевой счет?',
     'Внимание!',
     MB_YESNO+MB_ICONQUESTION) = IDYES then
  begin
    ...
    begin
      msg2('Лицевой счет успешно удален!',
...
begin
  if (msg3('Удалить в данном л/с услугу: '+
      ...
      OD_charge.FieldByName('NORM').AsString+
      '?',
     'Подверждение', MB_YESNO+MB_ICONQUESTION) = IDYES) then
  begin
    DataModule1.OraclePackage1.CallProcedure
         ('scott.p_houses.house_del_usl',
         [4, Form_list_kart.OD_list_kart.FieldByName('lsk').AsString,
         null,
         null,
         null,
         ...
         OD_charge.FieldByName('norm').AsFloat,
         1]);
...
procedure TForm_kart.wwDBGrid2CalcCellColors(Sender: TObject;
