object Form_print_pred_debit: TForm_print_pred_debit
  Left = 433
  Top = 564
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1054#1090#1095#1077#1090' '#1087#1086' '#1079#1072#1076#1086#1083#1078#1077#1085#1085#1086#1089#1090#1080' '#1087#1086' '#1087#1088#1077#1076#1087#1088#1080#1103#1090#1080#1103#1084
  ClientHeight = 117
  ClientWidth = 301
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object Label6: TLabel
    Left = 144
    Top = 24
    Width = 21
    Height = 13
    Caption = '       '
  end
  object GroupBox3: TGroupBox
    Left = 0
    Top = 65
    Width = 297
    Height = 49
    TabOrder = 3
    object Button4: TButton
      Left = 129
      Top = 16
      Width = 75
      Height = 25
      Caption = #1055#1088#1086#1089#1084#1086#1090#1088
      TabOrder = 0
      OnClick = Button4Click
    end
    object Button1: TButton
      Left = 214
      Top = 16
      Width = 75
      Height = 25
      Cancel = True
      Caption = #1054#1090#1084#1077#1085#1072
      TabOrder = 1
      OnClick = Button1Click
    end
  end
  object CheckBox1: TCheckBox
    Left = 32
    Top = 48
    Width = 249
    Height = 17
    Caption = #1059#1095#1080#1090#1099#1074#1072#1090#1100' '#1087#1086#1089#1090#1091#1087#1083#1077#1085#1080#1103' '#1090#1077#1082#1091#1097#1077#1075#1086' '#1087#1077#1088#1080#1086#1076#1072
    TabOrder = 2
  end
  object DBLookupComboBox1: TDBLookupComboBox
    Left = 16
    Top = 20
    Width = 113
    Height = 21
    KeyField = 'MENU_ID'
    ListField = 'NAME'
    ListSource = DS_perm_menu
    TabOrder = 1
    OnCloseUp = DBLookupComboBox1CloseUp
  end
  object DBLookupComboBox7: TDBLookupComboBox
    Left = 184
    Top = 19
    Width = 97
    Height = 21
    Enabled = False
    KeyField = 'KEY'
    ListField = 'REU'
    ListSource = DS_reu
    TabOrder = 0
  end
  object OD_data: TOracleDataSet
    SQL.Strings = (
      'begin'
      
        '  scott.generator.report_debit(:type_, :trest_, :psaldo_refcurso' +
        'r);'
      'end;')
    Optimize = False
    Variables.Data = {
      0300000003000000110000003A5053414C444F5F524546435552534F52740000
      000000000000000000060000003A545950455F03000000000000000000000007
      0000003A54524553545F050000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      04000000060000000500000053554D4D41010000000000030000004F52470100
      00000000040000004E414D450100000000000400000054595045010000000000
      080000004F52475F4E414D4501000000000003000000444154010000000000}
    Session = DataModule1.OracleSession1
  end
  object OD_perm_menu: TOracleDataSet
    SQL.Strings = (
      
        'select menu_id, name from scott.v_permissions_menu t where menu_' +
        'id in (0,1)')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      0400000002000000070000004D454E555F4944010000000000040000004E414D
      45010000000000}
    Session = DataModule1.OracleSession1
    Active = True
    Top = 32
  end
  object DS_perm_menu: TDataSource
    DataSet = OD_perm_menu
    Top = 64
  end
  object OD_reu_trest: TOracleDataSet
    SQL.Strings = (
      'select reu as key, reu from scott.s_reu_trest')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      040000000200000003000000524555010000000000030000004B455901000000
      0000}
    Session = DataModule1.OracleSession1
    Active = True
    Left = 32
    Top = 64
  end
  object DS_reu: TDataSource
    DataSet = OD_reu_trest
    Left = 64
    Top = 64
  end
  object frxDBDataset1: TfrxDBDataset
    UserName = 'frxDBDataset1'
    CloseDataSource = False
    DataSet = OD_data
    BCDToCurrency = False
    Left = 32
  end
  object frxReport1: TfrxReport
    Version = '4.7.71'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = #1055#1086' '#1091#1084#1086#1083#1095#1072#1085#1080#1102
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 38301.412938229200000000
    ReportOptions.LastChange = 38348.693871388900000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    ShowProgress = False
    Left = 64
    Datasets = <
      item
        DataSet = frxDBDataset1
        DataSetName = 'frxDBDataset1'
      end>
    Variables = <>
    Style = <>
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object MasterData1: TfrxMasterData
        Height = 22.677180000000000000
        Top = 226.771800000000000000
        Width = 718.110700000000000000
        DataSet = frxDBDataset1
        DataSetName = 'frxDBDataset1'
        RowCount = 0
        object Memo1: TfrxMemoView
          Left = 124.724490000000000000
          Width = 385.512060000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'NAME'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBDataset1."NAME"]')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Left = 68.031540000000000000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'ORG'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Memo.UTF8 = (
            '[frxDBDataset1."ORG"]')
        end
        object Memo3: TfrxMemoView
          Left = 517.795610000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'SUMMA'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8 = (
            '[frxDBDataset1."SUMMA"]')
        end
      end
      object ReportTitle1: TfrxReportTitle
        Height = 68.031540000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo4: TfrxMemoView
          Left = 90.708720000000000000
          Top = 3.779530000000001000
          Width = 582.047620000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Times New Roman'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            
              #1056#1115#1057#8218#1057#8225#1056#181#1057#8218' '#1056#1111#1056#1109' '#1056#183#1056#176#1056#1169#1056#1109#1056#187#1056#182#1056#181#1056#1029#1056#1029#1056#1109#1057#1027#1057#8218#1056#1105' '#1056#183#1056#176' '#1056#8211#1056#1113#1056#1032' '#1056#1111#1056#1109' '#1056#1111#1056#1109 +
              #1057#1027#1057#8218#1056#176#1056#1030#1057#8240#1056#1105#1056#1108#1056#176#1056#1112' '#1057#1107#1057#1027#1056#187#1057#1107#1056#1110' '#1056#1029#1056#176' [Dat_ #ddd.mm.yyyy]')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 170.078850000000000000
          Top = 34.015770000000010000
          Width = 389.291590000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            #1056#1111#1056#1109' [Org_]')
          ParentFont = False
        end
      end
      object Header1: TfrxHeader
        Height = 34.015770000000000000
        Top = 147.401670000000000000
        Width = 718.110700000000000000
        object Memo5: TfrxMemoView
          Left = 68.031540000000000000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Memo.UTF8 = (
            #1056#1113#1056#1109#1056#1169)
        end
        object Memo6: TfrxMemoView
          Left = 124.724490000000000000
          Width = 385.512060000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            #1056#1116#1056#176#1056#1105#1056#1112#1056#181#1056#1029#1056#1109#1056#1030#1056#176#1056#1029#1056#1105#1056#181' '#1056#1111#1056#1109#1057#1027#1057#8218#1056#176#1056#1030#1057#8240#1056#1105#1056#1108#1056#176)
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          Left = 517.795610000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          ShowHint = False
          HAlign = haRight
          Memo.UTF8 = (
            #1056#1038#1057#1107#1056#1112#1056#1112#1056#176' '#1056#1169#1056#1109#1056#187#1056#1110#1056#176' ('#1057#8218#1057#8249#1057#1027'.'#1057#1026#1057#1107#1056#177'.)')
        end
        object Line1: TfrxLineView
          Left = 64.252010000000000000
          Top = 26.456709999999990000
          Width = 612.283860000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
      end
      object ReportSummary1: TfrxReportSummary
        Height = 105.826840000000000000
        Top = 370.393940000000000000
        Width = 718.110700000000000000
        object Memo8: TfrxMemoView
          Left = 517.795610000000000000
          Top = 7.559059999999988000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          HAlign = haRight
          Memo.UTF8 = (
            '[SUM(<frxDBDataset1."SUMMA">,MasterData1)]')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 68.031540000000000000
          Top = 7.559059999999988000
          Width = 136.063080000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          Memo.UTF8 = (
            #1056#152#1057#8218#1056#1109#1056#1110#1056#1109' '#1056#1111#1056#1109' '#1056#1109#1057#8218#1057#8225#1056#181#1057#8218#1057#1107':')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Left = 64.252010000000000000
          Top = 3.779530000000022000
          Width = 612.283860000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
        object Memo10: TfrxMemoView
          Left = 71.811070000000000000
          Top = 79.370130000000010000
          Width = 196.535560000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          Memo.UTF8 = (
            #1056#8221#1056#1105#1057#1026#1056#181#1056#1108#1057#8218#1056#1109#1057#1026' '#1056#1114#1056#1119' '#1056#1032#1056#8226#1056#8212#1056#8211#1056#1113#1056#1032)
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 461.102660000000000000
          Top = 79.370130000000010000
          Width = 113.385900000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          HAlign = haRight
          Memo.UTF8 = (
            #1056#152#1056#187#1057#1039#1057#1027#1056#1109#1056#1030' '#1056#1115'.'#1056#8217'.')
          ParentFont = False
        end
      end
      object GroupHeader1: TfrxGroupHeader
        Top = 204.094620000000000000
        Width = 718.110700000000000000
        Condition = 'frxDBDataset1."TYPE"'
      end
      object GroupFooter1: TfrxGroupFooter
        Height = 37.795300000000000000
        Top = 272.126160000000000000
        Width = 718.110700000000000000
        object Memo13: TfrxMemoView
          Left = 68.031540000000000000
          Top = 3.779530000000022000
          Width = 162.519790000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          Memo.UTF8 = (
            #1056#152#1057#8218#1056#1109#1056#1110#1056#1109' '#1056#1111#1056#1109' '#1056#1030#1056#1105#1056#1169#1057#1107' '#1057#1107#1057#1027#1056#187#1057#1107#1056#1110':')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Left = 517.795610000000000000
          Top = 3.779530000000022000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          HAlign = haRight
          Memo.UTF8 = (
            '[SUM(<frxDBDataset1."SUMMA">,MasterData1)]')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = 64.252010000000000000
          Width = 612.283860000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
      end
    end
  end
  object frxXLSExport1: TfrxXLSExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    ExportEMF = True
    AsText = False
    Background = True
    FastExport = True
    PageBreaks = True
    EmptyLines = True
    SuppressPageHeadersFooters = False
    Left = 96
  end
  object frxRTFExport1: TfrxRTFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    ExportEMF = True
    Wysiwyg = True
    Creator = 'FastReport http://www.fast-report.com'
    SuppressPageHeadersFooters = False
    HeaderFooterMode = hfText
    AutoSize = False
    Left = 128
  end
end
